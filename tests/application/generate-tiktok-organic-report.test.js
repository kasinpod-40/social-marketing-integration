...
const DAILY_SETTING_KEY = `${PROFILE}:tiktok:daily`;
...
function dailyKey(id, date) {
  return `${contentKey(id)}:${date}`;
}
...
function buildRepository() {
  ...
  const tables = new Map(Object.entries({
    content: [{ recordId: 'content-1', fields: {
      content_key: contentKey('1'), external_content_id: '1', account_id: ACCOUNT,
    ...
    daily: [
      { recordId: 'daily-base', fields: {
        content_daily_key: dailyKey('1', '2026-07-10'), external_content_id: '1',
        account_id: ACCOUNT, platform: 'tiktok', metric_date: Date.parse('2026-07-09T17:00:00Z'),
        views: 100, likes: 10, comments: 2, shares: 1, avg_watch_time_seconds: 2,
        completion_rate: 0.2,
      } },
      { recordId: 'daily-current', fields: {
        content_daily_key: dailyKey('1', '2026-07-11'), external_content_id: '1',
        account_id: ACCOUNT, platform: 'tiktok', metric_date: Date.parse('2026-07-10T17:00:00Z'),
  ...
  return {
    ...
    async createMany(tableId, rows) {
      ...
      for (const fields of rows) {
        target.push({ recordId: `${tableId}-${nextId++}`, fields: structuredClone(fields) });
      }
...
function reportInput(repository, overrides = {}) {
  return {
    ...
    reportSettingKey: DAILY_SETTING_KEY,
  ...
  for (let id = 2; id <= total; id += 1) {
    ...
    repository.tables.get('daily').push(
      { recordId: `daily-${id}-base`, fields: {
        content_daily_key: dailyKey(id, '2026-07-10'),
        external_content_id: String(id), account_id: ACCOUNT, platform: 'tiktok',
      ...
      { recordId: `daily-${id}-current`, fields: {
        content_daily_key: dailyKey(id, '2026-07-11'),
        external_content_id: String(id), account_id: ACCOUNT, platform: 'tiktok',
...
test('generates idempotent daily TikTok report rows across all report tables', async () => {
  ...
  const first = await generateTikTokOrganicReport(reportInput(repository));
  ...
  const second = await generateTikTokOrganicReport(reportInput(repository));
  assert.deepEqual(
...
test('plans every output table before the first write', async () => {
  ...
  await assert.rejects(
    () => generateTikTokOrganicReport(reportInput(repository, { syncEngine })),
    /Schema invalid/,
  );
...
test('uses one bounded top-content limit for snapshot JSON and normalized rows', async () => {
  ...
  const result = await generateTikTokOrganicReport(reportInput(repository, { topContentLimit: 3 }));
  ...
  await assert.rejects(
    () => generateTikTokOrganicReport(reportInput(repository, { topContentLimit: 101 })),
...
test('neutralizes stale ranks when top-content limit is reduced', async () => {
  const repository = buildRepository();
  addReportableContent(repository, 5);
  await generateTikTokOrganicReport(reportInput(repository, { topContentLimit: 5 }));

  const second = await generateTikTokOrganicReport(reportInput(repository, {
    topContentLimit: 3,
    now: () => Date.parse('2026-07-12T02:00:00Z'),
  }));
...
function addHistoricalBaseline(repository, date = '2026-07-09') {
  repository.tables.get('daily').push({ recordId: `daily-old-${date}`, fields: {
    content_daily_key: dailyKey('1', date),
    external_content_id: '1', account_id: ACCOUNT, platform: 'tiktok',
...
test('server-filtered source produces the same report metrics as bounded full-page compatibility reads', async () => {
  ...
  const fallback = await generateTikTokOrganicReport(reportInput(fallbackRepository));
  const filtered = await generateTikTokOrganicReport(reportInput(filteredRepository));
...
test('bounded fallback cap fails before any report output plan or write', async () => {
  ...
  await assert.rejects(
    () => generateTikTokOrganicReport(reportInput(repository, {
      syncEngine,
