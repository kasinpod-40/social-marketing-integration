...
test('resolves active Settings from only the exact Fresh 7D source Reports', () => {
...
test('fails closed on inactive Settings or destination drift', () => {
  assert.throws(
    () => resolveLarkWeekly7dNotificationSourceSettings({
      sourceReportIds: ['report-current-7d'],
      snapshots: [snapshot('report-current-7d', 'setting-current-7d')],
      settings: [setting('setting-current-7d', { notification_enabled: false })],
      expectedDestinationKeyHash: DESTINATION_HASH,
    }),
