...
test('invalid-JSON recovery chain follows the proven diagnostics recovery completion order', async () => {
  const source = await readFile(
    new URL('../../scripts/woocommerce-invalid-json-recovery-chain.mjs', import.meta.url),
    'utf8',
  );
  const diagnostics = source.indexOf("'woocommerce-worker-provider-diagnostics'");
...
test('provider diagnostics must pass before lifecycle mutation is authorized', async () => {
  const source = await readFile(
    new URL('../../scripts/woocommerce-invalid-json-recovery-chain.mjs', import.meta.url),
    'utf8',
  );
  const diagnosticsCall = source.indexOf("'woocommerce-worker-provider-diagnostics'");
