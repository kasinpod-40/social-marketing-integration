...
test('invalid-JSON recovery chain follows preview-window diagnostics recovery completion order', async () => {
  const source = await readFile(
    new URL('../../scripts/woocommerce-invalid-json-recovery-chain.mjs', import.meta.url),
    'utf8',
  );
  const diagnostics = source.indexOf("'woocommerce-worker-provider-diagnostics-preview-window'");
  ...
  assert.match(source, /scripts\/woocommerce-worker-provider-diagnostics-preview-window\.mjs/u);
  assert.match(source, /OPEN_AND_RESTORE_WOOCOMMERCE_PREVIEW_URLS/u);
  assert.doesNotMatch(
    source,
    /\['scripts\/woocommerce-worker-provider-diagnostics(?:-ephemeral)?\.mjs', '--execute'\]/u,
  );
...
test('preview-window entrypoint owns subdomain discovery, ephemeral auth, and Safe restore', async () => {
  const source = await readFile(
...
test('invalid-JSON recovery chain is resumable after exact lifecycle recovery', async () => {
  ...
  assert.match(source, /previewWindowRestored: true/u);
});
...
test('preview-window provider diagnostics must pass before lifecycle mutation is authorized', async () => {
  const source = await readFile(
    new URL('../../scripts/woocommerce-invalid-json-recovery-chain.mjs', import.meta.url),
    'utf8',
  );
  const diagnosticsCall = source.indexOf("'woocommerce-worker-provider-diagnostics-preview-window'");
