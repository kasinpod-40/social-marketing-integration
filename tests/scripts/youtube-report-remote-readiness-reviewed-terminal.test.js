...
test('reviewed terminal validates repository before invoking the internal collector', () => {
  ...
  assert.match(reviewedSource, /youtube-report-live-readiness-reviewed-audit\.js/u);
});

test('reviewed terminal and internal collector preserve read-only safety boundaries', () => {
  ...
  assert.match(reviewedSource, /YOUTUBE_REPORT_REMOTE_COLLECTOR_INTERNAL_HANDOFF/u);
  assert.match(internalSource, /assertSelectOnlySql/u);
});
