...
test('allows only the developer-owned Integration Workspace for Shared-table schema operations', () => {
  ...
  assert.equal(runtime.infrastructureOwner, 'developer');

  const legacyAlias = assertSharedTableSchemaDevTarget({
    MKT_ENV: 'development',
    MKT_CUSTOMER_PROFILE: 'dev_ft_pumkin',
  });
  assert.equal(legacyAlias.profileKey, 'integration_workspace');
  assert.equal(legacyAlias.compatibilityAlias, 'dev_ft_pumkin');
