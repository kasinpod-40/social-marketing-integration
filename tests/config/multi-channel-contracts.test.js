...
test('Ads model covers shared hierarchy and prevents target ROAS from becoming actual ROAS', () => {
  assert.deepEqual(ADS_CANONICAL_CONTRACT.hierarchy, ['account', 'campaign', 'ad_group', 'creative']);
  ...
  assert.deepEqual(ADS_LARK_BLUEPRINT.map((table) => table.primaryField), [
    'ads_account_key', 'ads_campaign_key', 'ads_ad_group_key', 'ads_creative_key', 'ads_daily_key',
  ]);
});
