...
test('structured operational sanitization keeps allowlisted counters but redacts identities and secrets', () => {
  const sanitized = sanitizeOperationalValue({
    ...
    accountKey: 'customer_account',
    accountId: 123456789,
    pageId: 223456789,
    instagramAccountId: 323456789,
    adAccountId: 423456789,
    tableId: 'tbl_private',
    identity: false,
    apiToken: 'private',
    privateKey: 111111,
  ...
  assert.equal(sanitized.customerProfile, '[REDACTED]');
  assert.equal(sanitized.accountKey, '[REDACTED]');
  assert.equal(sanitized.accountId, '[REDACTED]');
  assert.equal(sanitized.pageId, '[REDACTED]');
  assert.equal(sanitized.instagramAccountId, '[REDACTED]');
  assert.equal(sanitized.adAccountId, '[REDACTED]');
  assert.equal(sanitized.tableId, '[REDACTED]');
  assert.equal(sanitized.identity, '[REDACTED]');
