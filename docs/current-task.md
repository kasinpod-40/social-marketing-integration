...
- map Chatwoot period-end Account snapshot metrics to canonical `current_total`;
- keep event and duration metrics as `period_delta`;
- preserve Report identity, metric keys, values, comparisons, dimensions and Stable keys;
- preserve the existing Lark schema and Select options;
- add an end-to-end regression through `buildReportMetricValueRows` proving that all 19 Chatwoot summary metrics use accepted canonical scopes;
- perform no Remote action during implementation.
