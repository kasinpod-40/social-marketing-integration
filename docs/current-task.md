...
`DASHBOARD_REPORT_CONFIGURATION_INVALID`; the Notification Runtime baseline was restored and no materialization,
Work or lock was created.
...
### Queue activation defect — current

The active Worker version and Report flags passed the existing deployment verifier, but the exact Queue job was
terminalized before `runReliableSync`. The prior verifier checked Worker version/bindings only and used a 30-second
barrier; it did not inspect the actual Queue consumer inventory. The main Queue permits a 30-second batch wait, so a
Worker deployment can appear stable before the Queue consumer has observed the execution window.
...
- retain the existing Report Finalizer, config window, deployment runtime, Queue sender, D1/Lark integrity and closure
  helpers;
- inspect `GET /accounts/{account}/queues/{queue}/consumers` before any send;
- require exactly one Worker consumer named `social-mkt-sync-worker` with reviewed batch, concurrency, retry, wait and
  DLQ settings;
- use three exact Worker-version samples over 120 seconds only for a Report execution window;
- keep the Notification baseline restore barrier at the existing 30 seconds;
- bind all three exact DLQs and the retained failed continuation attempt;
- permit one original Queue send and one exact replay only;
- close all three DLQs only after D1/Lark integrity, replay stability and baseline restore pass.
...
3. Safe Notification restore still uses three samples across 30s.
4. Current state contains exactly three bound open DLQs, six historical D1 failures, zero latest Sync Runs and an empty
   D1/Lark target.
...
Repository implementation is in progress. No Remote Worker, Queue, D1, Lark or Provider action has been performed by
this branch.
