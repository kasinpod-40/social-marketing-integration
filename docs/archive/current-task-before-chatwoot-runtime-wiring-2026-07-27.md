...
- fallback Remote code ที่หายหรือไม่อยู่ในรูป stable code เป็น `TIKTOK_POST_LARK_AUDIT_FAILED`
...
- Generic authenticated failure ตอบ HTTP `400` และ `code=TIKTOK_POST_LARK_AUDIT_FAILED`
- Known error code เช่น `LARK_TABLE_CONFIG_INVALID` ไม่ถูกเขียนทับ
- Wrong token ยังตอบ HTTP `401` และ `code=TIKTOK_POST_LARK_AUDIT_UNAUTHORIZED`
- Disabled route ยังตอบ `404`; non-GET ยังตอบ `405`
- Success ยังตอบ `200`, `mode=read_only` และไม่มี Queue/Business write
- Operator error สำหรับ non-`200` มี local code คงที่และ details เพียง `httpStatus` กับ `remoteCode`
