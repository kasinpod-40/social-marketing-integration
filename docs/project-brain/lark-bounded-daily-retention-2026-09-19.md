...
## Restoration checkpoint — 2026-09-25

Git audit found this approved implementation on `40ed9627` but absent from current main even while the
Production flag remained enabled. No scheduled six-table job was admitted on 24 or 25 September. The
restoration imports only this implementation and its config/catalog/scheduler/router/test wiring.
The old 1254002 DLQ remains open until an exact controlled six-table run succeeds and Lark readback
confirms its effect; never replay the old Queue message blindly.
