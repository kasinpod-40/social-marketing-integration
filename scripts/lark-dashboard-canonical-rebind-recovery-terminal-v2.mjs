...
try {
  ...
  attemptRoot = join(
    evidenceRoot,
    `recovery-v2-scope-preflight-${new Date().toISOString().replaceAll(':', '-')}`,
  );
  ...
  currentStage = 'dashboard-read-scope-probe';
  ...
  const preflight = Object.freeze({
    ...
    decision: 'LARK_DASHBOARD_RECOVERY_V2_READ_PREFLIGHT_PASS',
    requiredScopes: REQUIRED_LARK_DASHBOARD_RECOVERY_SCOPES,
    readScopeVerified: true,
    dashboardUpdateScopeDeclared: true,
    blockUpdateScopeDeclared: true,
  ...
  currentStage = 'confirm-complete-recovery-scope-contract';
  assertDashboardRecoveryScopeConfirmation(
    process.env.CONFIRM_LARK_DASHBOARD_RECOVERY_SCOPE_CONTRACT,
  );

  currentStage = 'launch-reviewed-recovery-operator';
  process.exitCode = await runRecoveryOperator({ repositoryRoot, evidenceRoot });
} catch (error) {
  const failure = Object.freeze({
    ok: false,
    contractVersion: LARK_DASHBOARD_CANONICAL_REBIND_RECOVERY_VERSION,
    stage: currentStage,
    code: error?.code ?? 'LARK_DASHBOARD_RECOVERY_V2_PREFLIGHT_FAILED',
...
async function runRecoveryOperator({ repositoryRoot, evidenceRoot }) {
  return new Promise((resolvePromise, rejectPromise) => {
    const child = spawn(
      process.execPath,
      [join(repositoryRoot, 'scripts', 'lark-dashboard-canonical-rebind-recovery-v2.mjs'), '--execute'],
      {
        cwd: repositoryRoot,
        env: {
          ...process.env,
    ...
    child.once('close', (code, signal) => {
      if (signal) {
        const error = new Error(`Dashboard recovery operator terminated by signal ${signal}`);
        error.code = 'LARK_DASHBOARD_RECOVERY_OPERATOR_SIGNALLED';
        rejectPromise(error);
        return;
      }
...
function requireText(value, fieldName) {
  if (typeof value !== 'string' || value.trim() === '') {
    ...
    error.code = 'LARK_DASHBOARD_RECOVERY_V2_VALUE_INVALID';
