...
const CONFIRMATION = 'TRIGGER_NATIVE_LARK_AI_WEEKLY_7D_UAT_V1';
const PREVIOUS_MARKER = 'CONTROLLED_UAT_RETRY_COMPACT_V1';
const RETRY_MARKER = 'CONTROLLED_UAT_NATIVE_AI_RETRY_V2';
...
try {
  ...
  process.stderr.write(`${JSON.stringify({
    ok: false,
    contractVersion: 'lark_native_ai_weekly_7d_native_action_retry_v1',
    stage,
    code: error?.code ?? 'LARK_NATIVE_AI_RETRY_FAILED',
...
function printPlan() {
  process.stdout.write(`${JSON.stringify({
    ...
    planOnly: true,
    contractVersion: 'lark_native_ai_weekly_7d_native_action_retry_v1',
    objective: 'trigger_exact_pending_compact_weekly_7d_uat_after_native_ai_action_switch',
...
async function execute() {
  stage = 'confirmation';
  if (process.env.CONFIRM_LARK_NATIVE_AI_WEEKLY_7D_NATIVE_ACTION_RETRY !== CONFIRMATION) {
    throw failure('Exact native AI retry confirmation is missing', 'LARK_NATIVE_AI_RETRY_CONFIRMATION_INVALID');
  }
  ...
  if (repository.branch !== 'main' || repository.clean !== true || repository.head !== repository.originMain) {
    throw failure('Native AI retry requires clean exact current main', 'LARK_NATIVE_AI_RETRY_REPOSITORY_INVALID', repository);
  }
  ...
  if (matches.length !== 1) {
    throw failure('Expected exactly one pending compact weekly Executive UAT row', 'LARK_NATIVE_AI_RETRY_UAT_ROW_INVALID', {
  ...
  stage = 'verify-compact-evidence';
  if (metricSummary.evidenceShape !== 'executive_business_first_v2'
    || metricSummary.promptShape !== 'lark_ai_compact_v1'
    ...
    throw failure('Existing weekly Executive UAT evidence is not the reviewed compact prompt shape', 'LARK_NATIVE_AI_RETRY_EVIDENCE_INVALID', {
      evidenceShape: optionalText(metricSummary.evidenceShape),
      promptShape: optionalText(metricSummary.promptShape),
      channelBusinessEvidenceCount: Array.isArray(metricSummary.channelBusinessEvidence) ? metricSummary.channelBusinessEvidence.length : null,
  ...
  const update = await client.batchUpdateRecords({
    tableId: aiTableId,
    records: [{ recordId, fields: { failure_code: RETRY_MARKER } }],
  });
  if (update.updated !== 1) {
    throw failure('Expected exactly one native AI retry trigger write', 'LARK_NATIVE_AI_RETRY_WRITE_COUNT_INVALID', { updated: update.updated });
  }
  ...
  for (let attempt = 1; attempt <= POLL_ATTEMPTS; attempt += 1) {
    ...
    if (rows.length !== 1) {
      throw failure('UAT row identity drifted while observing native AI retry', 'LARK_NATIVE_AI_RETRY_READBACK_INVALID', { count: rows.length });
    }
  ...
  const result = Object.freeze({
    ...
    contractVersion: 'lark_native_ai_weekly_7d_native_action_retry_v1',
    stage: 'complete',
    status: generated ? 'weekly_7d_native_ai_generated' : 'weekly_7d_native_ai_retry_not_completed',
    repository,
    automationState,
    evidenceLength: {
      metricSummaryChars: metricSummaryText.length,
...
async function verifyAutomationState(workflows) {
  ...
  if (aiHash !== expectedAi?.workflowIdSha256 || !ACTIVE.has(aiStatus)) {
    throw failure('Exact AI Materialization automation must be active', 'LARK_NATIVE_AI_RETRY_AI_AUTOMATION_NOT_ACTIVE', {
  ...
  if (notificationHash !== expectedNotification?.workflowIdSha256 || !INACTIVE.has(notificationStatus)) {
    throw failure('Exact Notification automation must remain inactive', 'LARK_NATIVE_AI_RETRY_NOTIFICATION_NOT_SAFE', {
...
function exactWorkflow(workflows, title) {
  ...
  if (matches.length !== 1) throw failure(`Expected one exact Automation: ${title}`, 'LARK_NATIVE_AI_RETRY_AUTOMATION_IDENTITY_INVALID', { title, count: matches.length });
  return matches[0];
}
...
function exactTableId(tables, name) {
  ...
  if (matches.length !== 1) throw failure(`Expected one exact Lark table: ${name}`, 'LARK_NATIVE_AI_RETRY_TABLE_INVALID', { name, count: matches.length });
...
async function loadRuntime() {
  ...
  if (config.name !== 'social-mkt-sync-worker'
    ...
    throw failure('Reviewed Integration Workspace Lark runtime is incomplete', 'LARK_NATIVE_AI_RETRY_RUNTIME_INVALID');
  }
  return env;
}
...
function parseJsonObject(value, label) {
  try {
    ...
    throw failure(`${label} must be a JSON object`, 'LARK_NATIVE_AI_RETRY_JSON_INVALID', { label });
  }
}
function parseJsonArray(value, label) {
  try {
    ...
    throw failure(`${label} must be a JSON array`, 'LARK_NATIVE_AI_RETRY_JSON_INVALID', { label });
  }
}
