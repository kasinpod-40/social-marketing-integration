...
try {
  ...
  tempDirectory = await mkdtemp(join(tmpdir(), 'meta-d1-wrangler-compat-'));
  ...
  process.stderr.write(`${JSON.stringify({
    ok: false,
    code: error?.code ?? 'META_D1_WRANGLER_COMPAT_LAUNCHER_FAILED',
...
function requireAbsolutePath(value, fieldName) {
  if (typeof value !== 'string' || value.trim() === '') {
    ...
    error.code = 'META_D1_WRANGLER_COMPAT_LAUNCHER_INPUT_REQUIRED';
    throw error;
  }
  const absolute = resolve(value.trim());
  if (absolute !== value.trim()) {
    const error = new Error(`${fieldName} must be an absolute path`);
    error.code = 'META_D1_WRANGLER_COMPAT_LAUNCHER_INPUT_INVALID';
    throw error;
  }
  return absolute;
}
