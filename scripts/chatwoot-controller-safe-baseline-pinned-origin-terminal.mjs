...
const WRAPPER_HEAD_ENV = 'MKT_CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_HEAD';
const INNER_WRAPPER_HEAD_ENV = 'MKT_CHATWOOT_SAFE_BASELINE_WRAPPER_HEAD';
...
const INNER_WRAPPER = 'scripts/chatwoot-controller-safe-baseline-resume-terminal.mjs';
const SOURCE_CONFIG = 'wrangler.sync.jsonc';
const SUCCESS_MARKER = 'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_COMPLETED_SAFE';
...
try {
  ...
  if (!execute) {
    ...
    stage = 'confirm-chatwoot-safe-baseline-pinned-origin';
    assertConfirmation(process.env);

    stage = 'verify-chatwoot-safe-baseline-pinned-origin-wrapper';
    const repository = verifyWrapperCheckout(process.env);

    stage = 'verify-chatwoot-safe-baseline-pinned-origin-assets';
    const assets = await verifyLocalAssets();

    stage = 'prepare-chatwoot-safe-baseline-pinned-origin';
    temporaryRoot = await mkdtemp(join(tmpdir(), 'mkt-chatwoot-safe-baseline-pinned-'));
    ...
    stage = 'run-chatwoot-safe-baseline-resume';
    childStarted = true;
  ...
  const failure = {
    ok: false,
    stage,
    code: error?.code ?? 'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_FAILED',
...
function parseArgs(args) {
  ...
  if (unknown.length > 0) {
    throw wrapperError(
      'Chatwoot safe-baseline pinned-origin wrapper accepts only --execute',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_ARGUMENT_INVALID',
      { unknown },
    );
  }
...
function printPlan() {
  process.stdout.write(`${JSON.stringify({
    ok: true,
    planOnly: true,
    contractVersion: 'chatwoot_safe_baseline_pinned_origin_v1',
...
function assertConfirmation(env) {
  if (env[CONFIRMATION_ENV] !== CONFIRMATION_VALUE) {
    throw wrapperError(
      `Chatwoot safe-baseline pinned-origin wrapper requires ${CONFIRMATION_ENV}=${CONFIRMATION_VALUE}`,
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_CONFIRMATION_REQUIRED',
...
function verifyWrapperCheckout(env) {
  ...
  if (currentHead !== expectedHead || dirty.trim() !== '') {
    throw wrapperError(
      'Chatwoot safe-baseline pinned-origin wrapper requires the exact clean reviewed commit',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_CHECKOUT_INVALID',
      {
        expectedHead,
        currentHead,
        originMain,
  ...
  if (!gitSuccess(repositoryRoot, ['merge-base', '--is-ancestor', currentHead, originMain])) {
    throw wrapperError(
      ...
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_ANCESTRY_INVALID',
  ...
  if (conflicts.length > 0) {
    throw wrapperError(
      'Chatwoot safe-baseline pinned-origin wrapper refuses caller-provided Git config overrides',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_GIT_CONFIG_ENV_INVALID',
...
async function writeExactExcludes(cloneRoot) {
  ...
  if (existing?.isSymbolicLink()) {
    throw wrapperError(
      'Safe-baseline pinned-origin clone exclude path must not be a symlink',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_EXCLUDE_INVALID',
    );
  }
  ...
  if (!info.isFile() || (info.mode & 0o077) !== 0 || observed !== expected) {
    throw wrapperError(
      'Safe-baseline pinned-origin clone exclude is not exact and private',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_EXCLUDE_INVALID',
    );
  }
}
...
async function verifyPreparedClone(cloneRoot, expectedHead) {
  ...
  if (fetch.error || fetch.status !== 0) {
    throw wrapperError(
      'Safe-baseline pinned-origin clone could not fetch its exact synthetic main',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_FETCH_INVALID',
      {
  ...
  if (observed.head !== expectedHead
      ...
      || observed.dirty.trim() !== '') {
    throw wrapperError(
      'Prepared Chatwoot safe-baseline pinned-origin clone is not exact and clean after fetch',
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_CLONE_INVALID',
      {
        head: observed.head,
...
async function assertDirectory(path, field) {
  ...
  throw wrapperError(
    ...
    'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_LOCAL_ASSET_MISSING',
    { field },
  );
}
...
async function assertRegularFile(path, field) {
  ...
  throw wrapperError(
    ...
    'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_LOCAL_FILE_INVALID',
    { field },
  );
}
...
async function assertPrivateRegularFile(path, field) {
  ...
  throw wrapperError(
    ...
    'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_LOCAL_PRIVATE_FILE_INVALID',
    { field },
  );
}
...
function requireSha(value, field) {
  ...
  if (!/^[0-9a-f]{40}$/u.test(text)) {
    throw wrapperError(
      ...
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_HEAD_ENV_INVALID',
      { field },
    );
  }
  return text;
...
function gitText(cwd, args, required = true) {
  ...
  if (result.status !== 0 && required) {
    throw wrapperError(
      ...
      'CHATWOOT_SAFE_BASELINE_PINNED_ORIGIN_GIT_READ_FAILED',
...
function wrapperError(message, code, details = {}) {
  ...
  error.name = 'ChatwootSafeBaselinePinnedOriginError';
  error.code = code;
