...
try {
  ...
  const exactUrl = new URL(META_K2_EXACT_RECOVERY_PATH, ingress.origin).toString();
...
async function fetchCloudflarePages(baseUrl, bearerToken, query = {}) {
  ...
  for (let page = 1; page <= totalPages; page += 1) {
    ...
    const observedTotalPages = Number(body.result_info?.total_pages ?? 1);
    if (!Number.isSafeInteger(observedTotalPages)
      || observedTotalPages < 1 || observedTotalPages > 20) {
      throw launcherError(
        'Cloudflare ingress pagination metadata is invalid',
