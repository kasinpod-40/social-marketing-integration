...
export function loadYouTubeLarkUatTarget(env = {}) {
  ...
  const target = {
    repositoryHead: requireSha(
      env.MKT_YOUTUBE_LARK_UAT_REPOSITORY_HEAD,
      'MKT_YOUTUBE_LARK_UAT_REPOSITORY_HEAD',
    ),
    ...
    accountKey: requireExact(
      env.MKT_YOUTUBE_LARK_UAT_ACCOUNT_KEY,
      'dev_ft_pumkin',
      'MKT_YOUTUBE_LARK_UAT_ACCOUNT_KEY',
    ),
    channelId: requireText(
      env.MKT_YOUTUBE_LARK_UAT_EXPECTED_CHANNEL_ID,
      'MKT_YOUTUBE_LARK_UAT_EXPECTED_CHANNEL_ID',
    ),
    workerName: requireExact(
      env.MKT_YOUTUBE_LARK_UAT_WORKER_NAME ?? 'social-mkt-sync-worker',
      'social-mkt-sync-worker',
      'MKT_YOUTUBE_LARK_UAT_WORKER_NAME',
    ),
    databaseName: requireExact(
      env.MKT_YOUTUBE_LARK_UAT_DATABASE_NAME ?? 'social-mkt-state-dev',
      'social-mkt-state-dev',
      'MKT_YOUTUBE_LARK_UAT_DATABASE_NAME',
    ),
    mainQueueName: requireExact(
      env.MKT_YOUTUBE_LARK_UAT_MAIN_QUEUE ?? 'social-mkt-sync-jobs',
      'social-mkt-sync-jobs',
      'MKT_YOUTUBE_LARK_UAT_MAIN_QUEUE',
    ),
    dlqName: requireExact(
      env.MKT_YOUTUBE_LARK_UAT_DLQ ?? 'social-mkt-sync-dlq',
      'social-mkt-sync-dlq',
      'MKT_YOUTUBE_LARK_UAT_DLQ',
    ),
    wranglerConfigPath: requireText(
      env.MKT_YOUTUBE_LARK_UAT_WRANGLER_CONFIG ?? 'wrangler.sync.jsonc',
      'MKT_YOUTUBE_LARK_UAT_WRANGLER_CONFIG',
    ),
...
export function buildYouTubeLarkUatConfigWindow(sourceText, input = {}) {
  ...
  for (const flag of [
    ...
    if (!flagNames.includes(flag)) {
      throw uatError(
        `YouTube Lark UAT source config lacks ${flag}`,
        'YOUTUBE_LARK_UAT_CONFIG_FLAG_MISSING',
        { flag },
      );
    }
  }
  ...
  const tableIds = Object.freeze(Object.fromEntries(
    Object.entries(LARK_ENV_BY_KEY).map(([key, envName]) => [
      key,
      requireRealMapping(source.vars?.[envName], envName),
    ]),
  ));
  const d1 = exactlyOne(
    source.d1_databases,
...
export function buildYouTubeLarkFullSyncJob(input = {}) {
  ...
  if (input.metricDate !== undefined && input.metricDate !== metricDate) {
    throw uatError(
      'YouTube Lark UAT metricDate must match generation in Asia/Bangkok',
      'YOUTUBE_LARK_UAT_METRIC_DATE_INVALID',
...
export function buildYouTubeLarkUatSnapshotSql(input = {}) {
  ...
  const syncRunId = sqlText(requireExact(
    input.syncRunId,
    `youtube-lark-uat:${input.operationId}`,
    'syncRunId',
  ));
  return compactSql(`
    SELECT
...
export function classifyYouTubeLarkUatCompletion(snapshotInput = {}) {
  ...
  if (snapshot.dlqRecords !== 0) {
    throw uatError('YouTube Lark UAT operation appeared in the DLQ', 'YOUTUBE_LARK_UAT_DLQ_DETECTED');
  }
  if (['failed', 'partial_success', 'skipped'].includes(snapshot.syncRunStatus)) {
    throw uatError(
      'YouTube Lark UAT Sync run reached a terminal non-success status',
      'YOUTUBE_LARK_UAT_SYNC_FAILED',
      { status: snapshot.syncRunStatus, errorCode: snapshot.syncRunErrorCode },
    );
  }
  const required = {
...
export function classifyYouTubeLarkCounts(counts = {}) {
  const normalized = Object.fromEntries(YOUTUBE_LARK_UAT_REQUIRED_TABLE_KEYS.map((key) => [
    key,
    nonNegativeInteger(counts[key] ?? 0, key),
  ]));
  const missingPositive = YOUTUBE_LARK_UAT_REQUIRED_POSITIVE_COUNT_KEYS
    .filter((key) => normalized[key] <= 0);
  return deepFreeze({
    counts: normalized,
    complete: missingPositive.length === 0,
    missingPositive,
...
export function compareYouTubeLarkUatRerun(input = {}) {
  ...
  if (changedLark.length > 0 || changedDurable.length > 0) {
    throw uatError(
      'YouTube Lark UAT rerun changed stable business counts',
      'YOUTUBE_LARK_UAT_IDEMPOTENCY_FAILED',
      { changedLark, changedDurable },
    );
  }
  if (after.mainQueueAttempts < 2) {
    throw uatError(
      'YouTube Lark UAT rerun was not admitted twice',
      'YOUTUBE_LARK_UAT_RERUN_NOT_OBSERVED',
    );
  }
  return deepFreeze({
    idempotent: true,
...
function validateBaseConfig(source, input) {
  ...
  if (source.workers_dev !== false) {
    throw uatError('YouTube Lark UAT requires workers_dev=false', 'YOUTUBE_LARK_UAT_WORKERS_DEV_INVALID');
  }
  if (Array.isArray(source.routes) && source.routes.length > 0) {
    throw uatError('YouTube Lark UAT requires no Worker routes', 'YOUTUBE_LARK_UAT_ROUTE_INVALID');
  }
...
function normalizeFlagWindow(config) {
  ...
  for (const key of Object.keys(copy.vars ?? {})) {
    if (/^MKT_[A-Z0-9_]+_ENABLED$/u.test(key)) copy.vars[key] = '<flag>';
  }
  return copy;
}
...
function requireOperationId(value) {
  ...
  if (!SAFE_OPERATION_ID.test(text)) {
    throw uatError('YouTube Lark UAT operationId is invalid', 'YOUTUBE_LARK_UAT_OPERATION_ID_INVALID');
  }
  return text;
}
...
function requireRealMapping(value, fieldName) {
  ...
  if (/replace|placeholder|example|todo|000000/iu.test(text)) {
    throw uatError(`${fieldName} is not a real mapping`, 'YOUTUBE_LARK_UAT_TABLE_MAPPING_INVALID');
  }
  return text;
}
...
function nonNegativeInteger(value, fieldName) {
  ...
  if (!Number.isSafeInteger(number) || number < 0) {
    throw uatError(`${fieldName} must be a non-negative integer`, 'YOUTUBE_LARK_UAT_COUNT_INVALID');
  }
  return number;
}
...
function requireText(value, fieldName) {
  if (typeof value !== 'string' || value.trim() === '') {
    throw uatError(`${fieldName} is required`, 'YOUTUBE_LARK_UAT_TARGET_INVALID', { fieldName });
  }
  return value.trim();
}
