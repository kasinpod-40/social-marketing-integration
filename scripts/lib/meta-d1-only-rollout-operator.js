...
export const META_D1_ONLY_CONFIRMATIONS = deepFreeze({
  preflight: confirmation(
    'CONFIRM_META_D1_ONLY_PREFLIGHT',
    'PREFLIGHT_META_D1_ONLY_ROLLOUT',
  ),
  backup: confirmation(
    'CONFIRM_META_D1_ONLY_BACKUP',
    'BACKUP_META_D1_ONLY_STATE',
  ),
  'deploy-safe-baseline': confirmation(
    'CONFIRM_META_D1_ONLY_DEPLOY_SAFE',
    'DEPLOY_META_D1_ONLY_SAFE_BASELINE',
  ),
  'verify-safe-baseline': confirmation(
    'CONFIRM_META_D1_ONLY_VERIFY_SAFE',
    'VERIFY_META_D1_ONLY_SAFE_BASELINE',
  ),
  'deploy-d1-only-gates': confirmation(
    'CONFIRM_META_D1_ONLY_DEPLOY_ACTIVE',
    'DEPLOY_META_D1_ONLY_GATES',
  ),
  'verify-d1-only-deployment': confirmation(
    'CONFIRM_META_D1_ONLY_VERIFY_ACTIVE',
    'VERIFY_META_D1_ONLY_DEPLOYMENT',
  ),
  'snapshot-before': confirmation(
    'CONFIRM_META_D1_ONLY_SNAPSHOT',
    'SNAPSHOT_META_D1_ONLY_BASELINE',
  ),
  'send-one-d1-only': confirmation(
    'CONFIRM_META_D1_ONLY_SEND',
    'SEND_ONE_META_D1_ONLY_OPERATION',
  ),
  'verify-d1-only': confirmation(
    'CONFIRM_META_D1_ONLY_VERIFY',
    'VERIFY_META_D1_ONLY_OPERATION',
  ),
  'resend-same-operation': confirmation(
    'CONFIRM_META_D1_ONLY_RESEND',
    'RESEND_SAME_META_D1_ONLY_OPERATION',
  ),
  'verify-idempotent-rerun': confirmation(
    'CONFIRM_META_D1_ONLY_VERIFY_RERUN',
    'VERIFY_META_D1_ONLY_IDEMPOTENT_RERUN',
  ),
  'restore-all-false': confirmation(
    'CONFIRM_META_D1_ONLY_RESTORE',
    'RESTORE_META_D1_ONLY_ALL_FALSE',
  ),
  'verify-restore': confirmation(
    'CONFIRM_META_D1_ONLY_VERIFY_RESTORE',
    'VERIFY_META_D1_ONLY_RESTORE',
  ),
  summary: confirmation(
    'CONFIRM_META_D1_ONLY_SUMMARY',
    'SUMMARIZE_META_D1_ONLY_ROLLOUT',
  ),
});
...
export const META_D1_ONLY_TARGETS = deepFreeze({
  facebook: {
    ...
    requiredSecretName: 'META_ACCESS_TOKEN',
    platform: 'facebook',
  },
  instagram: {
    connectorKey: 'instagram',
    connectorFlag: 'MKT_CONNECTOR_INSTAGRAM_ENABLED',
    jobType: JOB_TYPES.INSTAGRAM_ORGANIC_SYNC,
    sourceAccountKey: null,
    requiredSecretName: 'META_INSTAGRAM_ACCESS_TOKEN',
    platform: 'instagram',
  },
  chemistry_k2: {
    connectorKey: 'meta_ads',
    connectorFlag: 'MKT_CONNECTOR_META_ADS_ENABLED',
...
const EXECUTABLE_PHASES = new Set(
  META_D1_ONLY_OPERATOR_PHASES.filter((phase) => phase !== 'plan'),
);
const EVIDENCE_FILES = deepFreeze(Object.fromEntries(
  META_D1_ONLY_OPERATOR_PHASES.map((phase) => [phase, `${phase}.json`]),
));
...
export function parseMetaD1OnlyOperatorArgs(args = []) {
  ...
  for (const arg of args) {
    ...
    throw operatorError(
      `Unknown Meta D1-only rollout argument: ${arg}`,
      'META_D1_ONLY_OPERATOR_ARGUMENT_INVALID',
    );
  }
  if (!META_D1_ONLY_OPERATOR_PHASES.includes(phase)) {
    throw operatorError(
      `Unsupported Meta D1-only rollout phase: ${phase}`,
  ...
  if (phase === 'plan' && execute) {
    throw operatorError(
      ...
      'META_D1_ONLY_OPERATOR_PLAN_EXECUTE_INVALID',
    );
  }
...
export function assertMetaD1OnlyConfirmation(phase, env = {}) {
  ...
  if (env?.[expected.envName] !== expected.value) {
    throw operatorError(
      `Meta D1-only rollout requires ${expected.envName}=${expected.value}`,
      'META_D1_ONLY_OPERATOR_CONFIRMATION_REQUIRED',
...
export function loadMetaD1OnlyTarget(env = {}) {
  ...
  const repositoryHead = requireFullSha(
    env.MKT_META_D1_ONLY_REPOSITORY_HEAD,
    'MKT_META_D1_ONLY_REPOSITORY_HEAD',
  );
  ...
  if (periodStart > periodEnd) {
    throw operatorError(
      'Meta D1-only periodStart must not be after periodEnd',
      'META_D1_ONLY_PERIOD_INVALID',
    );
  }
  const workKey = definition.sourceAccountKey
    ? `meta_ads:${definition.sourceAccountKey}:${operationId}`
    : `${definition.connectorKey}:${operationId}`;
  ...
  const base = {
    ...
    accountKey: requireExact(
      env.MKT_META_D1_ONLY_ACCOUNT_KEY,
      'chemistry_k',
      'MKT_META_D1_ONLY_ACCOUNT_KEY',
    ),
    workerName: requireExact(
      env.MKT_META_D1_ONLY_WORKER_NAME,
      'social-mkt-sync-worker',
      'MKT_META_D1_ONLY_WORKER_NAME',
    ),
    databaseName: requireExact(
      env.MKT_META_D1_ONLY_DATABASE_NAME,
      'social-mkt-state-dev',
      'MKT_META_D1_ONLY_DATABASE_NAME',
    ),
    mainQueueName: requireExact(
      env.MKT_META_D1_ONLY_MAIN_QUEUE,
      'social-mkt-sync-jobs',
      'MKT_META_D1_ONLY_MAIN_QUEUE',
    ),
    dlqName: requireExact(
      env.MKT_META_D1_ONLY_DLQ,
      'social-mkt-sync-dlq',
      'MKT_META_D1_ONLY_DLQ',
    ),
    targetKey,
    ...
    expectedActiveVersion: requireVersionId(
      env.MKT_META_D1_ONLY_EXPECTED_ACTIVE_VERSION,
      'MKT_META_D1_ONLY_EXPECTED_ACTIVE_VERSION',
    ),
    wranglerConfigPath: requireText(
      env.MKT_META_D1_ONLY_WRANGLER_CONFIG,
      'MKT_META_D1_ONLY_WRANGLER_CONFIG',
    ),
    readOnlySummaryPath: requireText(
      env.MKT_META_D1_ONLY_READ_ONLY_SUMMARY,
  ...
  return deepFreeze({
    ...base,
    targetFingerprint: sha256(stableJson(safeTarget(base))),
  });
}
...
export function validateMetaReadOnlySummary(value = {}, target = {}) {
  if (value?.phase !== 'summary'
    || value?.status !== 'passed'
    || value?.contractVersion !== 'meta_read_only_validation_v1'
    || value?.details?.accepted !== true
    || Number(value?.details?.validationCount) !== 4
    || value?.mutationPerformed !== false
    || Number(value?.businessWrites) !== 0
    || Number(value?.queueMessages) !== 0) {
    throw operatorError(
      'Meta read-only summary is missing or not accepted',
      'META_D1_ONLY_READ_ONLY_SUMMARY_INVALID',
    );
  }
  const safe = value?.target ?? {};
  if (safe.environment !== target.environment
    || safe.customerProfile !== target.customerProfile
    || safe.customerKey !== target.customerKey
    || safe.executionFlagsEnabled !== false
    || safe.schedulesEnabled !== false) {
    throw operatorError(
      'Meta read-only summary target does not match the D1-only target',
      'META_D1_ONLY_READ_ONLY_TARGET_MISMATCH',
    );
  }
  const validations = Array.isArray(value?.details?.validations)
    ? value.details.validations
    : [];
  ...
  for (const [phase, [connectorKey, sourceAccountKey]] of expected) {
    ...
    if (entry?.connectorKey !== connectorKey
      || (entry?.sourceAccountKey ?? null) !== sourceAccountKey
      || entry?.status !== 'identity_validated'
      || !Number.isSafeInteger(Number(entry?.requestAttempts))
      || Number(entry.requestAttempts) < 1) {
      throw operatorError(
        `Meta read-only summary is incomplete for ${phase}`,
        'META_D1_ONLY_READ_ONLY_SUMMARY_INVALID',
        { phase },
      );
    }
  ...
  return deepFreeze({
    targetFingerprint: requireFingerprint(value.targetFingerprint, 'targetFingerprint'),
    summarySha256: sha256(stableJson(value)),
    validationCount: 4,
    nextGate: value?.details?.nextGate ?? null,
  });
}
...
export function buildMetaD1OnlyConfigWindow(safeText, target = {}) {
  ...
  if (safeTrueFlags.length !== 0) {
    throw operatorError(
      'Safe Meta D1-only config must have every MKT execution flag false',
      'META_D1_ONLY_SAFE_CONFIG_NOT_CLOSED',
      { trueFlags: safeTrueFlags },
    );
  }
  for (const flag of META_D1_ONLY_REQUIRED_FALSE_FLAGS) requireConfigBoolean(safe, flag, false);

  const activeFlags = Object.freeze([
    target.connectorFlag,
    'MKT_META_SOURCE_READ_ENABLED',
    'MKT_META_D1_WRITE_ENABLED',
  ].sort());
  let active = safe;
  ...
  if (JSON.stringify(observedActive) !== JSON.stringify(activeFlags)) {
    throw operatorError(
      'Meta D1-only active config contains an unapproved true flag',
      'META_D1_ONLY_ACTIVE_CONFIG_UNAPPROVED_FLAG',
      { trueFlags: observedActive },
    );
  }
  ...
  for (const flag of META_D1_ONLY_REQUIRED_FALSE_FLAGS) {
    if (!activeFlags.includes(flag)) requireConfigBoolean(active, flag, false);
  }
  if (normalizeApprovedFlagWindow(safe, activeFlags)
    !== normalizeApprovedFlagWindow(active, activeFlags)) {
    throw operatorError(
      'Meta D1-only config changes fields outside the approved flag window',
      'META_D1_ONLY_CONFIG_UNAPPROVED_DIFF',
    );
  }
  return deepFreeze({
    safeText: safe,
...
export function buildMetaD1OnlySchemaSql() {
  ...
  return compactSql(`
    SELECT
      ...
      GROUP_CONCAT(name, ',') AS required_table_names
    FROM sqlite_master
...
export function buildMetaD1OnlySnapshotSql(target = {}) {
  ...
  return compactSql(`
    SELECT
      ...
      (SELECT complete FROM sync_work_phases
        WHERE work_key = ${workKey} AND phase = '${D1_PHASE}') AS d1_phase_complete,
      (SELECT state_json FROM sync_work_phases
        WHERE work_key = ${workKey} AND phase = '${D1_PHASE}') AS d1_state_json,
      (SELECT COUNT(*) FROM sync_work_phases
        WHERE work_key = ${workKey} AND phase = '${LARK_PHASE}') AS lark_phase_count,
      (SELECT COUNT(*) FROM sync_work_phases
        WHERE work_key = ${workKey} AND phase = '${COMPLETION_PHASE}') AS completion_phase_count,
      (SELECT COUNT(*) FROM sync_locks
        WHERE owner_id = ${syncRunId} AND expires_at > (unixepoch() * 1000)) AS active_lock_count,
      (SELECT COUNT(*) FROM queue_operation_attempts
        WHERE operation_id = ${operationId} AND work_key = ${workKey}) AS queue_operation_attempts,
      (SELECT COALESCE(MAX(main_queue_attempts), 0) FROM queue_operation_attempts
        WHERE operation_id = ${operationId} AND work_key = ${workKey}) AS main_queue_attempts,
      (SELECT COUNT(*) FROM data_coverage_runs
        WHERE sync_run_id = ${syncRunId}) AS coverage_run_count,
      (SELECT COUNT(*) FROM data_coverage_runs
      ...
      (SELECT COUNT(*) FROM data_coverage_entities
        ...
        )) AS coverage_entity_count,
      (SELECT COUNT(*) FROM organic_content_state
        WHERE customer_key = ${customerKey} AND platform = ${platform}
          AND account_key = ${accountKey}) AS target_organic_state_count,
      (SELECT COUNT(*) FROM organic_content_observations
        WHERE customer_key = ${customerKey} AND platform = ${platform}
          AND account_key = ${accountKey}) AS target_organic_observation_count,
      (SELECT COUNT(*) FROM organic_account_daily_facts
        WHERE customer_key = ${customerKey} AND platform = ${platform}
          AND account_key = ${accountKey}) AS target_account_daily_count,
      (SELECT COUNT(*) FROM ads_entity_state
        WHERE customer_key = ${customerKey} AND platform = ${platform}
          AND account_key = ${accountKey}) AS target_ads_entity_count,
      (SELECT COUNT(*) FROM ads_daily_facts
        WHERE customer_key = ${customerKey} AND platform = ${platform}
          AND account_key = ${accountKey}) AS target_ads_daily_count,
      (SELECT COUNT(*) FROM organic_content_state
        WHERE last_sync_run_id = ${syncRunId}) AS operation_organic_state_count,
      (SELECT COUNT(*) FROM organic_content_observations
        WHERE sync_run_id = ${syncRunId}) AS operation_organic_observation_count,
      (SELECT COUNT(*) FROM organic_account_daily_facts
        WHERE sync_run_id = ${syncRunId}) AS operation_account_daily_count,
      (SELECT COUNT(*) FROM ads_entity_state
        WHERE last_sync_run_id = ${syncRunId}) AS operation_ads_entity_count,
      (SELECT COUNT(*) FROM ads_daily_facts
        WHERE sync_run_id = ${syncRunId}) AS operation_ads_daily_count;
  `);
}
...
export function classifyMetaD1OnlyCompletion(snapshot = {}) {
  ...
  const complete = value.syncRunStatus === 'success'
    ...
    && value.d1PhaseComplete === true
    && value.larkPhaseCount === 0
    && value.completionPhaseCount === 0
    && value.activeLockCount === 0
    && value.coverageRunCount > 0
    && value.invalidCoverageCount === 0
    && value.workLifecycleStatus === 'active'
    && value.workCompletedAt === null;
  return deepFreeze({
    complete,
    snapshot: value,
    reason: complete ? 'd1_complete_lark_gate_disabled' : 'incomplete_or_invalid',
  });
}
...
export function compareMetaD1OnlySnapshots(beforeInput, afterInput, options = {}) {
  ...
  if (after.larkPhaseCount !== 0 || after.completionPhaseCount !== 0) {
    throw operatorError(
      'Meta D1-only execution created a Lark or full-completion phase',
      'META_D1_ONLY_LARK_BOUNDARY_VIOLATED',
    );
  }
  if (after.invalidCoverageCount !== 0) {
    throw operatorError(
      'Meta D1-only Coverage contains failed or unaccepted rows',
      'META_D1_ONLY_COVERAGE_INVALID',
    );
  }
  if (!rerun) {
    const classified = classifyMetaD1OnlyCompletion(after);
    if (!classified.complete) {
      throw operatorError(
        'Meta D1-only operation has not reached the accepted D1 boundary',
        'META_D1_ONLY_COMPLETION_INVALID',
      );
    }
    if (after.queueOperationAttempts < before.queueOperationAttempts + 1) {
      throw operatorError(
        'Meta D1-only initial Queue attempt was not observed',
        'META_D1_ONLY_QUEUE_ATTEMPT_MISSING',
      );
    }
    return deepFreeze({
      accepted: true,
  ...
  if (after.queueOperationAttempts < before.queueOperationAttempts + 1) {
    throw operatorError(
      'Meta D1-only rerun Queue attempt was not observed',
      'META_D1_ONLY_RERUN_ATTEMPT_MISSING',
    );
  }
  for (const key of Object.keys(before.targetCounts)) {
    if (after.targetCounts[key] !== before.targetCounts[key]) {
      throw operatorError(
        'Meta D1-only rerun changed target Business counts',
        'META_D1_ONLY_RERUN_COUNT_DRIFT',
        { field: key, before: before.targetCounts[key], after: after.targetCounts[key] },
      );
    }
  }
  for (const key of Object.keys(before.operationCounts)) {
    if (after.operationCounts[key] !== before.operationCounts[key]) {
      throw operatorError(
        'Meta D1-only rerun changed operation-scoped Business counts',
        'META_D1_ONLY_RERUN_COUNT_DRIFT',
        { field: key, before: before.operationCounts[key], after: after.operationCounts[key] },
      );
    }
  }
  if (after.coverageRunCount !== before.coverageRunCount
    ...
    throw operatorError(
      'Meta D1-only rerun changed Coverage counts',
      'META_D1_ONLY_RERUN_COVERAGE_DRIFT',
    );
  }
  return deepFreeze({
    accepted: true,
...
export function validateMetaD1OnlyEvidenceSequence(evidence = [], target = {}) {
  if (!Array.isArray(evidence) || evidence.length === 0) {
    throw operatorError(
      'Meta D1-only evidence chain is empty',
      'META_D1_ONLY_EVIDENCE_MISSING',
    );
  }
  let previous = null;
  for (const item of evidence) {
    if (item?.contractVersion !== META_D1_ONLY_OPERATOR_CONTRACT_VERSION
      ...
      || item?.larkWritesAllowed !== false
      || item?.scheduleActivationAllowed !== false
      || item?.productionAllowed !== false) {
      throw operatorError(
        `Meta D1-only evidence is invalid for phase ${item?.phase ?? 'unknown'}`,
        'META_D1_ONLY_EVIDENCE_INVALID',
      );
    }
    const expectedHash = item.evidenceSha256;
    const unsigned = { ...item };
    delete unsigned.evidenceSha256;
    if (expectedHash !== sha256(stableJson(unsigned))) {
      throw operatorError(
        `Meta D1-only evidence hash is invalid for phase ${item.phase}`,
        'META_D1_ONLY_EVIDENCE_HASH_INVALID',
      );
    }
    if ((item.previousEvidenceSha256 ?? null) !== (previous?.evidenceSha256 ?? null)) {
      throw operatorError(
        `Meta D1-only evidence chain is broken at phase ${item.phase}`,
        'META_D1_ONLY_EVIDENCE_CHAIN_INVALID',
      );
    }
    previous = item;
  }
...
export function evidenceFileForMetaD1OnlyPhase(phase) {
  return EVIDENCE_FILES[requirePhase(phase)];
}
...
export function safeMetaD1OnlyTarget(target = {}) {
  return deepFreeze(safeTarget(target));
}
...
function safeTarget(target) {
  return {
    ...
    expectedActiveVersion: target.expectedActiveVersion,
    operationId: target.operationId,
    originalRequestedAt: target.originalRequestedAt,
...
function requirePhase(value) {
  if (!META_D1_ONLY_OPERATOR_PHASES.includes(value)) {
    throw operatorError('Meta D1-only phase is invalid', 'META_D1_ONLY_OPERATOR_PHASE_INVALID');
  }
  return value;
}
...
function requireTargetKey(value) {
  ...
  if (!SAFE_TARGET_KEY.test(key) || !META_D1_ONLY_TARGETS[key]) {
    throw operatorError(
      'Meta D1-only target must be facebook, instagram, chemistry_k2 or chemistry_k3',
      'META_D1_ONLY_TARGET_INVALID',
    );
  }
  return key;
}
...
function requireOperationId(value, fieldName) {
  ...
  if (!SAFE_OPERATION_ID.test(text)) {
    throw operatorError(
      `${fieldName} has an unsafe format`,
      'META_D1_ONLY_OPERATION_ID_INVALID',
      { fieldName },
    );
  }
  return text;
...
function requireFullSha(value, fieldName) {
  ...
  if (!FULL_SHA.test(text)) {
    throw operatorError(`${fieldName} must be a full Git SHA`, 'META_D1_ONLY_GIT_SHA_INVALID');
  }
  return text;
}
...
function requireVersionId(value, fieldName) {
  ...
  if (!VERSION_ID.test(text)) {
    throw operatorError(`${fieldName} must be a Worker version UUID`, 'META_D1_ONLY_VERSION_INVALID');
  }
  return text;
}
...
function requireFingerprint(value, fieldName) {
  ...
  if (!/^[0-9a-f]{64}$/u.test(text)) {
    throw operatorError(`${fieldName} must be a SHA-256 fingerprint`, 'META_D1_ONLY_FINGERPRINT_INVALID');
  }
  return text;
}
...
function requireTimestamp(value, fieldName) {
  ...
  if (!Number.isSafeInteger(number) || number < Date.UTC(2000, 0, 1)) {
    throw operatorError(`${fieldName} must be a valid timestamp`, 'META_D1_ONLY_TIMESTAMP_INVALID');
  }
  return number;
}
...
function requireDate(value, fieldName) {
  const text = requireText(value, fieldName);
  if (!DATE_ONLY.test(text) || Number.isNaN(Date.parse(`${text}T00:00:00Z`))) {
    throw operatorError(`${fieldName} must be YYYY-MM-DD`, 'META_D1_ONLY_DATE_INVALID');
  }
  return text;
}
...
function requireExact(value, expected, fieldName) {
  if (value !== expected) {
    throw operatorError(
      `Meta D1-only rollout requires ${fieldName}=${expected}`,
      'META_D1_ONLY_TARGET_INVALID',
      { fieldName },
    );
  }
...
function requireConfigString(text, key, expected) {
  const observed = readConfigString(text, key);
  if (observed !== expected) {
    throw operatorError(
      `Meta D1-only config requires ${key}=${expected}`,
...
function requireConfigBoolean(text, key, expected) {
  const value = readConfigBoolean(text, key);
  if (value !== expected) {
    throw operatorError(
      `Meta D1-only config requires ${key}=${expected}`,
      'META_D1_ONLY_CONFIG_INVALID',
...
function replaceConfigBoolean(text, key, expected) {
  ...
  if (!regex.test(text)) {
    throw operatorError(
      `Meta D1-only config is missing ${key}`,
      'META_D1_ONLY_CONFIG_INVALID',
      { key },
    );
  }
...
function sanitizeEvidenceValue(value) {
  ...
  for (const [key, nested] of Object.entries(value)) {
    ...
    result[key] = sanitizeEvidenceValue(nested);
  }
  return result;
}
...
function count(value) {
  ...
  if (!Number.isSafeInteger(number) || number < 0) {
    throw operatorError('Meta D1-only snapshot count is invalid', 'META_D1_ONLY_SNAPSHOT_INVALID');
  }
  return number;
}
