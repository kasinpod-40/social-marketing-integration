...
export function buildChatwootCurrentIncidentClosureSql(identity = {}, input = {}) {
  ...
  return compactSql(`
    ...
    WHERE operation_id=${operationId} AND main_queue_attempts IN (2,4,5,7,9,11,14) AND recovery_status='not_started';
    ...
    WHERE platform='chatwoot' AND status='open' AND (
      ...
      OR (sync_run_id=${sqlText(`${operation.syncRunId}:unit:2`)} AND error_code='UNHANDLED_SYNC_ERROR'
...
export function classifyChatwootInitialRecoveryBoundary(inspection = {}) {
  ...
  if (!original && !fractional && !race && !waiting && !unknownLabel && !messageOrder && !reportingEvent) {
