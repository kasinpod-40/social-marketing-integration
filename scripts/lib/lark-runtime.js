...
export async function createLocalLarkRuntime(requiredTableKeys) {
  ...
  const repository = new LarkRecordRepository({ client: createLarkBitableClientFromEnv(normalizedEnv) });
  const tables = readLarkTableIdsFromEnv(normalizedEnv, requiredTableKeys);
  return Object.freeze({ env: normalizedEnv, repository, tables });
}
...
export function readAccountId(env) {
  ...
  if (typeof value !== 'string' || value.trim() === '') {
    throw new Error('Missing TIKTOK_CREATOR_ACCOUNT_ID in .dev.vars or process env');
  }
  return value.trim();
}
