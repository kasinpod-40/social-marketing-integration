...
try {
  if (process.env[META_K2_SOURCE_COMPLETE_PREVIEW_MODE_ENV]
    ...
    || process.env[META_K2_SOURCE_COMPLETE_PREVIEW_V4_MODE_ENV]
      !== META_K2_SOURCE_COMPLETE_PREVIEW_V4_MODE) {
    const error = new Error('Meta K2 source-complete finalizer v4 mode is not authorized');
    error.code = 'META_K2_SOURCE_COMPLETE_FINALIZER_V4_MODE_REQUIRED';
    throw error;
  }
  register(
  ...
  process.stderr.write(`${JSON.stringify({
    ok: false,
    stage: 'meta-k2-source-complete-finalizer-bootstrap-v4',
    code: error?.code ?? 'META_K2_SOURCE_COMPLETE_FINALIZER_BOOTSTRAP_V4_FAILED',
