...
async function main() {
  ...
  const mode = resolveConfirmedApplyMode({
    ...
    env: process.env,
    operationName: 'Meta DEV schema apply',
    confirmationErrorCode: 'META_SCHEMA_WRITE_CONFIRMATION_REQUIRED',
    applyCommand: 'CONFIRM_WRITE=YES npm run setup:meta-schema:apply',
  });
  const [inventoryCsv, ...fieldCsvs] = await Promise.all([
    readFile(new URL('docs/meta-blueprint-v0.12.0/table-inventory.csv', ROOT), 'utf8'),
    ...FIELD_CONTRACTS.map((path) => readFile(new URL(path, ROOT), 'utf8')),
  ]);
  ...
  const common = {
    ...
    env,
    schema,
    schemaVersion: META_LARK_SCHEMA_VERSION,
    validateSchema: validateMetaLarkSchema,
  };
  ...
  if (!mode.apply) {
    ...
    printJson({
      ...
      nextCommand: preview.readyToApply
        ? 'CONFIRM_WRITE=YES npm run setup:meta-schema:apply'
        : null,
      note: 'Preview เท่านั้น ยังไม่สร้างหรือแก้ Meta/Facebook/Instagram tables ใน Lark DEV',
  ...
  printJson({
    ...result,
    target: { environment: runtime.environment, profileKey: runtime.profileKey },
    configNote: 'นำ environmentUpdates ไปใส่ใน local .dev.vars/wrangler.sync.jsonc เท่านั้น ห้าม Commit',
    safetyNote: 'Schema apply ไม่เขียนข้อมูลธุรกิจและไม่เปิด Meta connectors/schedules',
  });
}
