...
import {
  LARK_NATIVE_AI_DESTINATION_BINDING_CONFIRMATION,
  LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS,
  LARK_NATIVE_AI_DESTINATION_BINDING_OUTPUT_ROOT,
...
import {
  ...
  bindingError,
} from './lib/lark-native-ai-destination-binding.js';

const execFileAsync = promisify(execFile);
const repositoryRoot = resolve(process.cwd());
const configPath = resolve(
  process.env.MKT_LARK_NATIVE_AI_DESTINATION_BINDING_WRANGLER_CONFIG
    ?? 'wrangler.sync.jsonc',
);
const devVarsPath = resolve(process.env.DEV_VARS_FILE ?? '.dev.vars');
const outputRoot = resolve(
  process.env.MKT_LARK_NATIVE_AI_DESTINATION_BINDING_OUTPUT_ROOT
    ?? LARK_NATIVE_AI_DESTINATION_BINDING_OUTPUT_ROOT,
);
const lockPath = resolve(outputRoot, '.destination-binding.lock');
...
try {
  ...
  else await executeBinding();
} catch (error) {
  const counters = remote?.snapshot?.() ?? null;
  const failure = Object.freeze({
    ok: false,
    contractVersion: LARK_NATIVE_AI_DESTINATION_BINDING_VERSION,
    stage,
    code: error?.code ?? 'LARK_NATIVE_AI_DESTINATION_BINDING_FAILED',
    ...
    recordWriteCount: counters?.recordWriteCount ?? 0,
...
function printPlan() {
  process.stdout.write(`${JSON.stringify({
    ...
    planOnly: true,
    contractVersion: LARK_NATIVE_AI_DESTINATION_BINDING_VERSION,
    objective: 'bind_the_exact_visible_lark_chat_id_to_empty_integration_workspace_group_id_fields',
    exactCommand: [
      ...
      `CONFIRM_LARK_NATIVE_AI_DESTINATION_BINDING=${LARK_NATIVE_AI_DESTINATION_BINDING_CONFIRMATION}`,
      'node scripts/lark-native-ai-destination-binding-terminal.mjs --execute',
    ].join(' '),
    targetGroupName: LARK_NATIVE_AI_TARGET_GROUP_NAME,
    mutationBoundary: 'settings_group_id_only',
    maximumRecordWrites: LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumRecordWrites,
    workflowCreateCount: 0,
    workflowUpdateCount: 0,
    workflowStatusChangeCount: 0,
    automationEnabled: false,
...
async function executeBinding() {
  stage = 'confirmation';
  if (process.env.CONFIRM_LARK_NATIVE_AI_DESTINATION_BINDING
    !== LARK_NATIVE_AI_DESTINATION_BINDING_CONFIRMATION) {
    throw bindingError(
      'Exact Lark Native AI destination binding confirmation is missing',
      'LARK_NATIVE_AI_DESTINATION_BINDING_CONFIRMATION_INVALID',
    );
  }
  const nodeMajor = Number(process.versions.node.split('.')[0]);
  if (!Number.isInteger(nodeMajor) || nodeMajor < 22) throw bindingError(
    'Node.js 22 or newer is required',
    'LARK_NATIVE_AI_DESTINATION_BINDING_NODE_VERSION_UNSUPPORTED',
    { nodeMajor },
  );
  ...
  if (repository.branch !== 'main' || repository.clean !== true
    ...
    throw bindingError(
      'Destination binding requires clean current main',
      'LARK_NATIVE_AI_DESTINATION_BINDING_REPOSITORY_INVALID',
      repository,
    );
  }
  ...
  stage = 'bind-live-destination';
  remote = createDestinationBindingFetchGuard(globalThis.fetch.bind(globalThis));
  const rawClient = createLarkBitableClientFromEnv(Object.freeze({
    ...
    onRequest: (event) => process.stderr.write(`${JSON.stringify({
      stage: 'lark_destination_binding',
      event: sanitizeValue(event),
    })}\n`),
  });
  const client = createDestinationBindingClient(rawClient);
  const result = await applyLarkNativeAiDestinationBinding({ client });

  stage = 'verify-remote-boundary';
  const counters = remote.snapshot();
  if (counters.blocked !== 0
    || counters.recordWriteCount !== result.recordWriteCount
    || counters.batchUpdate > LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumBatchUpdateRequests) {
    throw bindingError(
      'Destination binding Remote counters did not match the reviewed boundary',
      'LARK_NATIVE_AI_DESTINATION_BINDING_REMOTE_COUNTER_INVALID',
      {
        blocked: counters.blocked,
        remoteRecordWriteCount: counters.recordWriteCount,
        resultRecordWriteCount: result.recordWriteCount,
        batchUpdate: counters.batchUpdate,
      },
    );
  }

  stage = 'write-binding-summary';
  const summary = Object.freeze({
    ...
    recordWriteCount: result.recordWriteCount,
    workflowCreateCount: 0,
    workflowUpdateCount: 0,
    workflowStatusChangeCount: 0,
    automationEnabled: false,
...
async function loadAndValidateRuntime() {
  ...
  try {
    ...
    throw bindingError(
      ...
      'LARK_NATIVE_AI_DESTINATION_BINDING_CONFIG_INVALID',
  ...
  if (blockers.length > 0) throw bindingError(
    'Destination binding local preflight found blockers',
    'LARK_NATIVE_AI_DESTINATION_BINDING_LOCAL_PREFLIGHT_BLOCKED',
    { blockerCount: blockers.length, blockers },
  );
  return Object.freeze({ config, env });
}

function createDestinationBindingClient(rawClient) {
  ...
  return Object.freeze({
    ...
    listFields(input) {
      if (!fieldPromise) fieldPromise = rawClient.listFields(input);
      return fieldPromise;
    },
    listRecords: rawClient.listRecords.bind(rawClient),
    batchUpdateRecords: rawClient.batchUpdateRecords.bind(rawClient),
    listChats() {
      if (!chatPromise) chatPromise = listChats(rawClient);
      return chatPromise;
    },
  });
}
...
async function listChats(client) {
  const items = [];
  let pageToken = null;
  for (let page = 1; page <= LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumChatListReads; page += 1) {
    ...
    if (!Array.isArray(pageItems)) throw bindingError(
      'Lark chat inventory returned invalid items',
      'LARK_NATIVE_AI_DESTINATION_BINDING_CHAT_INVENTORY_INVALID',
    );
    ...
    if (!next || next === pageToken) throw bindingError(
      'Lark chat inventory returned invalid pagination',
      'LARK_NATIVE_AI_DESTINATION_BINDING_CHAT_PAGINATION_INVALID',
      { page },
    );
    pageToken = next;
  }
  throw bindingError(
    ...
    'LARK_NATIVE_AI_DESTINATION_BINDING_CHAT_PAGE_LIMIT_EXCEEDED',
  );
}

export function createDestinationBindingFetchGuard(fetchImpl) {
  ...
  const limits = {
    token: 2,
    tableRead: LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumTableReads,
    fieldRead: LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumFieldReads,
    recordRead: LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumRecordReads,
    chatListRead: LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumChatListReads,
    batchUpdate: LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumBatchUpdateRequests,
  };
  ...
  const batchUpdatePath = /^\/open-apis\/bitable\/v1\/apps\/[^/]+\/tables\/[^/]+\/records\/batch_update$/u;
  ...
  async function guardedFetch(url, options = {}) {
    ...
    else if (method === 'POST' && batchUpdatePath.test(path)) kind = 'batchUpdate';

    if (!kind) {
      counters.blocked += 1;
      throw bindingError(
        'Destination binding request is outside the reviewed allowlist',
        'LARK_NATIVE_AI_DESTINATION_BINDING_REQUEST_BLOCKED',
    ...
    if (kind === 'batchUpdate') {
      ...
      if (!Array.isArray(records) || records.length === 0
        || records.length > LARK_NATIVE_AI_DESTINATION_BINDING_LIMITS.maximumRecordWrites) {
        counters.blocked += 1;
        throw bindingError(
          'Destination binding batch size is outside the reviewed boundary',
          'LARK_NATIVE_AI_DESTINATION_BINDING_WRITE_LIMIT_EXCEEDED',
          { observed: Array.isArray(records) ? records.length : null },
        );
      }
      ...
      for (const record of records) {
        ...
        const keys = fields && typeof fields === 'object' && !Array.isArray(fields)
          ? Object.keys(fields).sort()
          : [];
        if (!recordId || recordIds.has(recordId)
          || keys.length !== 1 || keys[0] !== 'group_id'
          || !optionalText(fields.group_id)) {
          counters.blocked += 1;
          throw bindingError(
            'Destination binding attempted an unreviewed Record mutation',
            'LARK_NATIVE_AI_DESTINATION_BINDING_WRITE_BODY_INVALID',
          );
        }
        recordIds.add(recordId);
      }
      counters.recordWriteCount += records.length;
    }
    ...
    if (counters[kind] > limits[kind]) {
      counters.blocked += 1;
      throw bindingError(
        'Destination binding request limit exceeded',
        'LARK_NATIVE_AI_DESTINATION_BINDING_REQUEST_LIMIT_EXCEEDED',
...
async function acquireLock() {
  try {
    ...
    if (error?.code === 'EEXIST') throw bindingError(
      'A destination binding lock already exists and is never deleted automatically',
      'LARK_NATIVE_AI_DESTINATION_BINDING_LOCK_EXISTS',
...
async function runGit(args) {
  try {
    ...
    throw bindingError(
      ...
      'LARK_NATIVE_AI_DESTINATION_BINDING_GIT_FAILED',
...
function sanitizePath(path) {
  return String(path)
    ...
    .replace(/\/records\/[^/]+/u, '/records/[redacted]');
}
