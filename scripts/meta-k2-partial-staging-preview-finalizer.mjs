...
try {
  ...
  process.stderr.write(`${JSON.stringify({
    ok: false,
    stage: currentStage,
    code: error?.code ?? 'META_K2_PARTIAL_STAGING_PREVIEW_FINALIZER_FAILED',
...
async function execute() {
  ...
  previewAlias = requireText(baseEnv.MKT_META_K2_PREVIEW_ALIAS, 'MKT_META_K2_PREVIEW_ALIAS');
  accountWorkersDevSubdomain = requireText(
    baseEnv.MKT_META_K2_PREVIEW_SUBDOMAIN,
    'MKT_META_K2_PREVIEW_SUBDOMAIN',
  );
  previewRecoveryUrl = requireRecoveryUrl(baseEnv.MKT_META_K2_EXACT_RECOVERY_URL);
  requireExact(
    previewRecoveryUrl,
    buildMetaK2PreviewRecoveryUrl({
      previewAlias,
  ...
  requireExact(
    baseEnv.MKT_META_K2_PRODUCTION_BASELINE_VERSION,
    productionBaselineVersion,
    'MKT_META_K2_PRODUCTION_BASELINE_VERSION',
  );
...
async function runD1Window(input) {
  ...
  const activePath = join(recoveryRoot, 'wrangler.meta-k2-d1.preview.jsonc');
  const safePath = join(recoveryRoot, 'wrangler.meta-k2.safe-preview.jsonc');
  ...
  try {
    ...
    const deployed = await uploadPreviewVersion(
      baseEnv,
      activePath,
      'meta-k2-exact-d1-preview-continuation',
    );
    activated = true;
    ...
    if (!completed) {
      throw finalizerFailure(
        'Bounded direct D1 Preview continuation did not complete',
    ...
    if (activated) {
      ...
      const restored = await uploadPreviewVersion(
        baseEnv,
        safePath,
        'meta-k2-preview-safe-close-after-d1',
      );
...
async function runLarkWindow(input) {
  ...
  const activePath = join(recoveryRoot, 'wrangler.meta-k2-lark.preview.jsonc');
  const safePath = join(recoveryRoot, 'wrangler.meta-k2.safe-lark-preview.jsonc');
  await writePrivateText(activePath, config.activeText);
  await writePrivateText(safePath, config.safeText);
  ...
  try {
    ...
    const deployed = await uploadPreviewVersion(
      baseEnv,
      activePath,
      'meta-k2-exact-lark-preview-continuation',
    );
    activated = true;
    ...
    if (!completed) {
      throw finalizerFailure(
        'Bounded direct Lark Preview continuation did not complete',
    ...
    if (activated) {
      ...
      const restored = await uploadPreviewVersion(
        baseEnv,
        safePath,
        'meta-k2-preview-safe-close-after-lark',
      );
...
function runLocalGates(env) {
  ...
  runVisible(process.execPath, [
    ...
    'tests/application/meta-k2-preview-recovery.test.js',
  ], env);
...
async function invokeExactContinuation(input) {
  ...
  try {
    ...
    if (!response.ok || !value) {
      throw finalizerFailure(
        `Meta exact Preview continuation failed with HTTP ${response.status}`,
...
async function uploadPreviewVersion(env, configPath, message) {
  ...
  try {
    ...
    requireExact(
      new URL(upload.previewOrigin).origin,
      new URL(previewRecoveryUrl).origin,
      'previewOrigin',
    );
    const productionCurrentVersion = await readActiveVersion(env, configPath);
    const transport = validateMetaK2PreviewTransport({
      productionBaselineVersion,
...
async function verifyVersionFlags(env, configPath, versionId, expectedTrueFlags) {
  ...
  if (stableJson(observed) !== stableJson([...expectedTrueFlags].sort())) {
    throw finalizerFailure(
      'Worker Preview execution flags differ from the exact reviewed window',
...
function assertExactQueueAttempts(snapshot) {
  if (snapshot.queueOperationAttempts !== EXACT.queueOperationAttempts
    ...
    throw finalizerFailure(
      'Meta K2 direct Preview continuation changed Queue attempts',
...
function finalizerFailure(message, code, details = {}) {
  ...
  error.name = 'MetaK2PartialStagingPreviewFinalizerError';
  error.code = code;
