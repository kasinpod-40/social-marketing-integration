...
const CONTRACT_VERSION = 'lark_native_ai_weekly_7d_quality_gate_v8';
const CONFIRMATION = 'RETRY_WEEKLY_7D_NATIVE_AI_QUALITY_GATE_V8';
...
try {
  ...
  process.stderr.write(`${JSON.stringify({
    ...
    stage,
    code: error?.code ?? 'LARK_NATIVE_AI_QUALITY_GATE_V8_FAILED',
...
function printPlan() {
  process.stdout.write(`${JSON.stringify({
    ...
    planOnly: true,
    contractVersion: CONTRACT_VERSION,
    objective: 'repair_false_positive_quality_gate_and_require_real_business_metric_in_summary_without_lark_prompt_change',
...
async function execute() {
  stage = 'confirmation';
  if (process.env.CONFIRM_LARK_NATIVE_AI_WEEKLY_7D_QUALITY_GATE_V8 !== CONFIRMATION) {
    throw failure('Exact Executive Writer V8 retry confirmation is missing', 'LARK_NATIVE_AI_QUALITY_GATE_V8_CONFIRMATION_INVALID');
  }
  if (LARK_NATIVE_AI_AUTOMATION_PROMPT_VERSION !== EXPECTED_PROMPT_VERSION) {
    throw failure('Repository Prompt contract must remain Prompt v3', 'LARK_NATIVE_AI_QUALITY_GATE_V8_PROMPT_CONTRACT_INVALID', {
  ...
  if (repository.branch !== 'main' || repository.clean !== true || repository.head !== repository.originMain) {
    throw failure('Executive Writer V8 requires clean exact current main', 'LARK_NATIVE_AI_QUALITY_GATE_V8_REPOSITORY_INVALID', repository);
  }
  ...
  stage = 'upgrade-quality-evidence-v5';
  const upgraded = upgradeLarkNativeAiExecutiveWriterEvidence({
  ...
  const priorQualityGate = validateLarkNativeAiExecutiveWriterOutputs(priorOutputs, upgraded.evidence);
  if (priorQualityGate.passed
    || priorQualityGate.violations.length !== 1
    || priorQualityGate.violations[0] !== 'insight_missing_business_metric_value') {
    throw failure(
      'V7 output must revalidate to the single confirmed missing-business-metric violation before V8 retry',
      'LARK_NATIVE_AI_QUALITY_GATE_V8_PRIOR_DIAGNOSIS_INVALID',
      { priorQualityGate },
    );
  }
  ...
  const preparationUpdate = await client.batchUpdateRecords({
    ...
    records: [{
      recordId,
      fields: {
        metric_summary_json: upgraded.metricSummaryJson,
  ...
  if (preparationUpdate.updated !== 1) {
    throw failure('Expected exactly one V8 preparation write', 'LARK_NATIVE_AI_QUALITY_GATE_V8_PREPARATION_WRITE_COUNT_INVALID', {
      updated: preparationUpdate.updated,
    });
  }

  stage = 'verify-v8-prepared-state';
  ...
  if (preparedRows.length !== 1) {
    throw failure('V8 prepared row identity drifted', 'LARK_NATIVE_AI_QUALITY_GATE_V8_PREPARED_READBACK_INVALID', {
      count: preparedRows.length,
    });
  }
  const prepared = requireObject(preparedRows[0].fields, 'prepared.fields');
  if (!isExactPreparedV8Row(prepared, upgraded.metricSummaryJson, channelStatusVectorText)) {
    throw failure('V8 row did not converge to the reviewed prepared state', 'LARK_NATIVE_AI_QUALITY_GATE_V8_PREPARED_STATE_INVALID', {
      generationStatus: optionalText(prepared.generation_status),
      promptShape: readPromptShape(prepared.metric_summary_json),
      failureCode: optionalText(prepared.failure_code),
  ...
  stage = 'trigger-v8-failure-code-only';
  ...
  if (triggerUpdate.updated !== 1) {
    throw failure('Expected exactly one failure_code-only V8 trigger write', 'LARK_NATIVE_AI_QUALITY_GATE_V8_TRIGGER_WRITE_COUNT_INVALID', {
      updated: triggerUpdate.updated,
    });
  }

  stage = 'observe-v8-ai-result';
  ...
  for (let attempt = 1; attempt <= POLL_ATTEMPTS; attempt += 1) {
    ...
    if (rows.length !== 1) {
      throw failure('V8 UAT row identity drifted while observing AI result', 'LARK_NATIVE_AI_QUALITY_GATE_V8_RESULT_READBACK_INVALID', {
        count: rows.length,
      });
    }
  ...
  const qualityGate = generated
    ? validateLarkNativeAiExecutiveWriterOutputs(outputs, upgraded.evidence)
  ...
  const result = Object.freeze({
    ...
    stage: 'complete',
    status: passed
      ? 'weekly_7d_native_ai_executive_writer_quality_v8_passed'
      : generated
        ? 'weekly_7d_native_ai_executive_writer_quality_v8_failed'
        : 'weekly_7d_native_ai_executive_writer_quality_v8_not_completed',
    repository,
    automationState,
    promptContract: {
    ...
    evidence: {
      ...
      afterMetricSummaryChars: upgraded.metricSummaryChars,
      channelStatusVectorChars: upgraded.channelStatusVectorChars,
      sourcePromptShape: SOURCE_PROMPT_SHAPE,
      promptShape: TARGET_PROMPT_SHAPE,
      evidenceRewriteCount: 1,
      businessEvidenceChannelCount: upgraded.evidence.businessEvidenceChannelCount,
      comparisonEvidenceChannelCount: upgraded.evidence.comparisonEvidenceChannelCount,
      businessEvidenceChannelNames: upgraded.evidence.businessEvidenceChannelNames,
      summaryRequiredFacts: upgraded.evidence.summaryRequiredFacts,
    },
    priorQualityGate,
    recordWriteCount: 2,
...
function isExactV7GeneratedRow(fields) {
...
async function verifyAutomationState(workflows) {
  ...
  if (aiHash !== expectedAi?.workflowIdSha256 || !ACTIVE.has(aiStatus)) {
    throw failure('Exact AI Materialization automation must be active', 'LARK_NATIVE_AI_QUALITY_GATE_V8_AI_AUTOMATION_INVALID', {
  ...
  if (notificationHash !== expectedNotification?.workflowIdSha256 || !INACTIVE.has(notificationStatus)) {
    throw failure('Exact Notification automation must remain inactive', 'LARK_NATIVE_AI_QUALITY_GATE_V8_NOTIFICATION_UNSAFE', {
...
function exactWorkflow(workflows, title) {
  ...
  if (matches.length !== 1) throw failure(`Expected one exact Automation: ${title}`, 'LARK_NATIVE_AI_QUALITY_GATE_V8_AUTOMATION_IDENTITY_INVALID', { title, count: matches.length });
  return matches[0];
}
...
function exactTableId(tables, name) {
  ...
  if (matches.length !== 1) throw failure(`Expected one exact Lark table: ${name}`, 'LARK_NATIVE_AI_QUALITY_GATE_V8_TABLE_INVALID', { name, count: matches.length });
...
async function loadRuntime() {
  ...
  if (config.name !== 'social-mkt-sync-worker'
    ...
    throw failure('Reviewed Integration Workspace Lark runtime is incomplete', 'LARK_NATIVE_AI_QUALITY_GATE_V8_RUNTIME_INVALID');
  }
  return env;
}
