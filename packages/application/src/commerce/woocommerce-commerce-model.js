...
export async function normalizeWooCommerceDataset(input = {}) {
  ...
  const normalize = {
    [WOOCOMMERCE_DATASETS.STORE]: normalizeStore,
    [WOOCOMMERCE_DATASETS.ORDERS]: normalizeOrder,
    [WOOCOMMERCE_DATASETS.PRODUCTS]: normalizeProduct,
    [WOOCOMMERCE_DATASETS.VARIATIONS]: normalizeVariation,
    [WOOCOMMERCE_DATASETS.CATEGORIES]: normalizeCategory,
    [WOOCOMMERCE_DATASETS.CUSTOMERS]: normalizeCustomer,
    [WOOCOMMERCE_DATASETS.COUPONS]: normalizeCoupon,
  }[dataset];
  if (dataset === WOOCOMMERCE_DATASETS.STORE) {
    appendRowSet(output, await normalize(records[0] ?? {}, context));
  } else {
    for (const record of records) appendRowSet(output, await normalize(record, context));
  }
...
export function parseWooCommerceMoneyMicros(value, fieldName = 'money') {
  ...
  if (!match) throw contractError(`${fieldName} must be a decimal string with at most six decimals`, fieldName);
  const micros = (BigInt(match[2]) * 1_000_000n)
...
export function classifyWooCommerceOrderStatus(value) {
  ...
  if (['cancelled', 'trash'].includes(status)) return 'cancelled';
  if (status === 'failed') return 'failed';
  return 'unknown';
}

export function createWooCommerceIncrementalBoundary(input = {}) {
  const watermark = nullableTimestamp(input.sourceWatermark);
  if (watermark === null) return null;
  const overlapSeconds = boundedInteger(input.overlapSeconds ?? 300, 'overlapSeconds', 0, 86_400);
  return new Date(Math.max(0, watermark - overlapSeconds * 1000)).toISOString();
}

export function calculateWooCommerceReport(input = {}) {
  const orders = Array.isArray(input.orders) ? input.orders : [];
  const lines = Array.isArray(input.lines) ? input.lines : [];
  const requestedCurrency = nullableText(input.currency)?.toUpperCase() ?? null;
  const selectedOrders = requestedCurrency ? orders.filter((row) => row.currency === requestedCurrency) : orders;
  ...
  for (const line of lines) {
    if (requestedCurrency && line.currency !== requestedCurrency) continue;
    ...
    row.quantity_ordered += integerOrZero(line.quantity);
    row.gross_sales_micros += integerOrZero(line.gross_sales_micros);
    row.discount_micros += integerOrZero(line.discount_micros);
    row.refund_micros += integerOrZero(line.refund_micros);
    row.net_sales_micros += integerOrZero(line.net_sales_micros);
    if (line.order_key) row.orderKeys.add(line.order_key);
    products.set(key, row);
  }
  const productRows = [...products.values()].map(({ orderKeys, ...row }) => Object.freeze({
    ...row,
    recognized_orders: orderKeys.size,
  })).sort((a, b) => b.net_sales_micros - a.net_sales_micros || a.product_key.localeCompare(b.product_key));
  return Object.freeze({
    currency: requestedCurrency ?? [...currencies][0] ?? null,
...
async function normalizeStore(store, context) {
  const sanitized = {
    ...
    currency: nullableText(store.currency)?.toUpperCase() ?? context.defaultCurrency,
  ...
  return {
    raw_commerce_stores: [{
      ...
      base_url_hash: await createStableFingerprint(nullableText(store.homeUrl ?? store.siteUrl) ?? context.accountKey),
      wc_version: sanitized.wc_version,
      wp_version: sanitized.wp_version,
      timezone: sanitized.timezone,
      currency: sanitized.currency,
      number_of_decimals: sanitized.number_of_decimals,
    ...
    commerce_store_state: [{
      ...
      wc_version: sanitized.wc_version,
      wp_version: sanitized.wp_version,
      reporting_timezone: sanitized.timezone,
      default_currency: sanitized.currency,
      number_of_decimals: sanitized.number_of_decimals,
      last_observed_at: context.fetchedAt,
      source_payload_hash: hash,
...
async function normalizeOrder(order, context) {
  ...
  const currency = requireCurrency(order.currency, 'order.currency');
  const sourceCreatedAt = requiredTimestamp(order.date_created_gmt ?? order.date_created, 'order.date_created');
  const sourceModifiedAt = requiredTimestamp(order.date_modified_gmt ?? order.date_modified, 'order.date_modified');
  ...
  const refunds = readRefunds(orderId, context.refundsByOrderId, order.refunds);
  const grossSales = sum(lineItems, (line) => money(line.subtotal) + money(line.subtotal_tax));
  const discount = money(order.discount_total) + money(order.discount_tax);
  const shipping = money(order.shipping_total) + money(order.shipping_tax);
  const tax = money(order.total_tax);
  const refund = sum(refunds, (row) => Math.abs(money(row.total)));
  const total = money(order.total);
  const net = grossSales - discount - refund;
  const status = nullableText(order.status)?.toLowerCase() ?? 'unknown';
  const statusClass = classifyWooCommerceOrderStatus(status);
  const recognizedCount = statusClass === 'recognized' ? 1 : 0;
  const provisionalCount = statusClass === 'provisional' ? 1 : 0;
  const recognizedRevenue = recognizedCount ? total - refund : 0;
  const methods = normalizeShippingMethods(order.shipping_lines);
  const sanitized = sanitizeOrder(order, {
    customerId, customerType, methods, grossSales, discount, shipping, tax, refund, total,
  });
  const hash = await createStableFingerprint(sanitized);
  const rowSet = {
    raw_commerce_orders: [{
      raw_order_key: orderKey,
      customer_key: context.customerKey,
      account_key: context.accountKey,
      external_order_id: String(orderId),
      order_number: nullableText(order.number),
      status,
      currency,
      ...
      payment_method_id: nullableText(order.payment_method),
      payment_method_title: nullableText(order.payment_method_title),
      shipping_method_ids_json: JSON.stringify(methods.ids),
      shipping_method_titles_json: JSON.stringify(methods.titles),
      gross_sales_micros: grossSales,
      discount_micros: discount,
      shipping_micros: shipping,
      tax_micros: tax,
    ...
    commerce_order_state: [{
      ...
      discount_micros: discount,
      refund_micros: refund,
      net_sales_micros: net,
      shipping_micros: shipping,
      tax_micros: tax,
      order_total_micros: total,
      recognized_revenue_micros: recognizedRevenue,
      recognized_order_count: recognizedCount,
      provisional_order_count: provisionalCount,
      line_item_count: lineItems.length,
      quantity_total: sum(lineItems, (line) => integerOrZero(line.quantity)),
      source_payload_hash: hash,
      last_coverage_run_id: context.coverageRunId,
      last_sync_run_id: context.syncRunId,
      created_at: context.now,
    ...
    impactedCustomers: customerId
      ? [customerAggregateKey(context.accountKey, customerId, currency)]
      : [],
  };
  const refundByLine = buildRefundByLine(refunds);
  for (const line of lineItems) mergeRowSets(rowSet, await normalizeOrderLine(line, {
    ...context,
    orderKey,
    orderId,
    metricDate,
  ...
  for (const refundRow of refunds) mergeRowSets(rowSet, await normalizeRefund(refundRow, {
    ...context,
    orderKey,
    orderId,
    currency,
...
async function normalizeOrderLine(line, context) {
  ...
  return {
    raw_commerce_order_items: [{
      raw_order_item_key: lineKey,
      raw_order_key: context.orderKey,
      customer_key: context.customerKey,
      account_key: context.accountKey,
      external_order_id: String(context.orderId),
      external_line_item_id: String(lineId),
      external_product_id: productId === null ? null : String(productId),
      external_variation_id: variationId === null ? null : String(variationId),
      sku: nullableText(line.sku),
      product_name: nullableText(line.name),
      quantity: integerOrZero(line.quantity),
      subtotal_micros: subtotal,
      subtotal_tax_micros: subtotalTax,
      total_micros: total,
      total_tax_micros: totalTax,
      taxes_json: JSON.stringify(sanitizeTaxes(line.taxes)),
      source_payload_hash: hash,
      ...audit(context),
    }],
...
async function normalizeRefund(refund, context) {
  ...
  const sanitized = sanitizeRefund(refund);
  const hash = await createStableFingerprint(sanitized);
  return {
    raw_commerce_refunds: [{
      raw_refund_key: `${context.orderKey}:${refundId}`,
      raw_order_key: context.orderKey,
      customer_key: context.customerKey,
      account_key: context.accountKey,
      external_order_id: String(context.orderId),
      external_refund_id: String(refundId),
      refund_micros: Math.abs(money(refund.total)),
      currency: context.currency,
      ...
      line_items_json: JSON.stringify(sanitizeRefundLines(refund.line_items)),
      source_payload_hash: hash,
      payload_json: JSON.stringify(sanitized),
      ...audit(context),
    }],
...
async function normalizeProduct(product, context) {
  ...
  const currency = optionalCurrency(product.currency ?? context.defaultCurrency);
  ...
  const common = {
    ...
    product_type: nullableText(product.type),
    sku: nullableText(product.sku),
    product_name: nullableText(product.name),
    status: nullableText(product.status),
    catalog_visibility: nullableText(product.catalog_visibility),
    currency,
    price_micros: nullableMoney(product.price),
    regular_price_micros: nullableMoney(product.regular_price),
    sale_price_micros: nullableMoney(product.sale_price),
    stock_status: nullableText(product.stock_status),
    stock_quantity: nullableInteger(product.stock_quantity),
    manage_stock: booleanInteger(product.manage_stock),
  ...
  return {
    raw_commerce_products: [{ raw_product_key: key, ...common, payload_json: JSON.stringify(sanitized), ...audit(context) }],
    commerce_product_state: [{
      ...
      product_type: common.product_type,
      sku: common.sku,
      product_name: common.product_name,
      status: common.status,
      catalog_visibility: common.catalog_visibility,
      currency,
      price_micros: common.price_micros,
      regular_price_micros: common.regular_price_micros,
      sale_price_micros: common.sale_price_micros,
      stock_status: common.stock_status,
      stock_quantity: common.stock_quantity,
      manage_stock: common.manage_stock,
      category_ids_json: common.category_ids_json,
      attributes_json: JSON.stringify(sanitizeAttributes(product.attributes)),
      source_created_at: common.source_created_at,
      source_modified_at: common.source_modified_at,
      source_payload_hash: hash,
      last_coverage_run_id: context.coverageRunId,
      last_sync_run_id: context.syncRunId,
      created_at: context.now,
...
async function normalizeVariation(variation, context) {
  ...
  const currency = optionalCurrency(variation.currency ?? context.defaultCurrency);
  const sanitized = sanitizeVariation(variation);
  const hash = await createStableFingerprint(sanitized);
  const common = {
    customer_key: context.customerKey,
    account_key: context.accountKey,
    external_product_id: String(parentId),
    external_variation_id: String(id),
    sku: nullableText(variation.sku),
    status: nullableText(variation.status),
    currency,
    price_micros: nullableMoney(variation.price),
    regular_price_micros: nullableMoney(variation.regular_price),
    sale_price_micros: nullableMoney(variation.sale_price),
    stock_status: nullableText(variation.stock_status),
    stock_quantity: nullableInteger(variation.stock_quantity),
    manage_stock: booleanInteger(variation.manage_stock),
    attributes_json: JSON.stringify(sanitizeAttributes(variation.attributes)),
  ...
  return {
    raw_commerce_product_variations: [{
      raw_variation_key: key,
      raw_product_key: parentKey,
      ...common,
      payload_json: JSON.stringify(sanitized),
      ...audit(context),
    }],
    commerce_product_state: [{
      ...
      manage_stock: common.manage_stock,
      category_ids_json: '[]',
      attributes_json: common.attributes_json,
      source_created_at: common.source_created_at,
      source_modified_at: common.source_modified_at,
      source_payload_hash: hash,
      last_coverage_run_id: context.coverageRunId,
...
async function normalizeCategory(category, context) {
  ...
  const sanitized = {
    id,
    parent: nullablePositiveInteger(category.parent) ?? 0,
    name: nullableText(category.name),
    slug: nullableText(category.slug),
    display: nullableText(category.display),
    menu_order: nullableInteger(category.menu_order),
    count: nullableInteger(category.count),
  };
  return {
    raw_commerce_categories: [{
      ...
      external_parent_id: sanitized.parent ? String(sanitized.parent) : null,
      category_name: sanitized.name,
      slug: sanitized.slug,
      display: sanitized.display,
      menu_order: sanitized.menu_order,
      product_count: sanitized.count,
      source_payload_hash: await createStableFingerprint(sanitized),
      payload_json: JSON.stringify(sanitized),
      ...audit(context),
    }],
  };
...
async function normalizeCustomer(customer, context) {
  ...
  const currency = requireCurrency(context.defaultCurrency, 'customer.currency');
  const rawKey = `woocommerce:${context.accountKey}:customer:${id}`;
  const aggregateKey = customerAggregateKey(context.accountKey, id, currency);
  const sanitized = {
    id,
    role: nullableText(customer.role),
    orders_count: nullableInteger(customer.orders_count),
  ...
  const hash = await createStableFingerprint(sanitized);
  const createdAt = nullableTimestamp(customer.date_created_gmt ?? customer.date_created);
  const modifiedAt = nullableTimestamp(customer.date_modified_gmt ?? customer.date_modified);
  return {
    raw_commerce_customers: [{
      raw_customer_key: rawKey,
      customer_key: context.customerKey,
      account_key: context.accountKey,
      external_customer_id: String(id),
      customer_type: 'registered',
      orders_count: sanitized.orders_count,
      total_spent_micros: nullableMoney(sanitized.total_spent),
      currency,
    ...
    commerce_customer_aggregates: [{
      customer_aggregate_key: aggregateKey,
      customer_key: context.customerKey,
      account_key: context.accountKey,
      platform: 'woocommerce',
      external_customer_id: String(id),
...
async function normalizeCoupon(coupon, context) {
  ...
  const currency = optionalCurrency(context.defaultCurrency);
  const codeHash = await createStableFingerprint(nullableText(coupon.code) ?? `coupon:${id}`);
  const sanitized = {
    id,
    code_hash: codeHash,
    discount_type: nullableText(coupon.discount_type),
    amount: nullableText(coupon.amount),
    usage_count: nullableInteger(coupon.usage_count),
    individual_use: coupon.individual_use === true,
  ...
  return {
    raw_commerce_coupons: [{
      ...
      customer_key: context.customerKey,
      account_key: context.accountKey,
      external_coupon_id: String(id),
      coupon_code_hash: codeHash,
      discount_type: sanitized.discount_type,
      amount_micros: nullableMoney(sanitized.amount),
      currency,
      usage_count: sanitized.usage_count,
      individual_use: booleanInteger(sanitized.individual_use),
...
function appendRowSet(output, rowSet) {
  const paths = {
    raw_commerce_stores: ['raw', 'stores'],
    raw_commerce_orders: ['raw', 'orders'],
    raw_commerce_order_items: ['raw', 'orderItems'],
    raw_commerce_products: ['raw', 'products'],
    raw_commerce_product_variations: ['raw', 'variations'],
    raw_commerce_categories: ['raw', 'categories'],
    raw_commerce_customers: ['raw', 'customers'],
    raw_commerce_coupons: ['raw', 'coupons'],
    raw_commerce_refunds: ['raw', 'refunds'],
    commerce_order_state: ['canonical', 'orders'],
    commerce_order_status_observations: ['canonical', 'orderStatusObservations'],
    commerce_order_line_facts: ['canonical', 'orderLines'],
    commerce_product_state: ['canonical', 'products'],
    commerce_customer_aggregates: ['canonical', 'customers'],
    commerce_daily_sales_facts: ['daily', 'sales'],
    commerce_product_daily_facts: ['daily', 'products'],
  };
  for (const [table, rows] of Object.entries(rowSet ?? {})) {
    if (!Array.isArray(rows) || !paths[table]) continue;
    output.d1RowsByTable[table] ??= [];
    output.d1RowsByTable[table].push(...rows);
    const [group, list] = paths[table];
    output[group][list].push(...rows);
  }
...
function mergeRowSets(target, source) {
...
function freezeOutput(output) {
  const freezeGroup = (group) => Object.freeze(Object.fromEntries(
    Object.entries(group).map(([key, rows]) => [key, Object.freeze(rows.map(freezeRow))]),
  ));
  return Object.freeze({
    raw: freezeGroup(output.raw),
    canonical: freezeGroup(output.canonical),
    daily: freezeGroup(output.daily),
    d1RowsByTable: Object.freeze(Object.fromEntries(
      Object.entries(output.d1RowsByTable).map(([table, rows]) => [table, Object.freeze(rows.map(freezeRow))]),
    )),
...
function normalizeContext(input) {
  ...
  if (fetchedAt === null || now === null) throw contractError('fetchedAt and now must be valid timestamps', 'timestamp');
  return Object.freeze({
...
function sanitizeOrder(order, derived) {
  return {
    id: order.id,
    number: nullableText(order.number),
    status: nullableText(order.status),
    currency: nullableText(order.currency),
    date_created_gmt: nullableText(order.date_created_gmt),
    date_modified_gmt: nullableText(order.date_modified_gmt),
    date_paid_gmt: nullableText(order.date_paid_gmt),
    date_completed_gmt: nullableText(order.date_completed_gmt),
    customer_type: derived.customerType,
    customer_id: derived.customerId,
    payment_method: nullableText(order.payment_method),
    payment_method_title: nullableText(order.payment_method_title),
    shipping_method_ids: derived.methods.ids,
    shipping_method_titles: derived.methods.titles,
    gross_sales_micros: derived.grossSales,
    discount_micros: derived.discount,
    shipping_micros: derived.shipping,
    tax_micros: derived.tax,
    refund_micros: derived.refund,
    total_micros: derived.total,
    line_items: (Array.isArray(order.line_items) ? order.line_items : []).map(sanitizeLine),
    coupon_ids: (Array.isArray(order.coupon_lines) ? order.coupon_lines : [])
      .map((line) => nullablePositiveInteger(line?.id)).filter(Boolean),
    refund_ids: (Array.isArray(order.refunds) ? order.refunds : [])
      .map((row) => nullablePositiveInteger(row?.id)).filter(Boolean),
  };
}
...
function sanitizeLine(line) {
  return {
    id: line?.id,
    product_id: nullablePositiveInteger(line?.product_id),
    variation_id: nullablePositiveInteger(line?.variation_id),
    sku: nullableText(line?.sku),
    name: nullableText(line?.name),
    quantity: nullableInteger(line?.quantity),
    subtotal: nullableText(line?.subtotal),
    subtotal_tax: nullableText(line?.subtotal_tax),
    total: nullableText(line?.total),
    total_tax: nullableText(line?.total_tax),
...
function sanitizeProduct(product) {
  return {
    id: product.id,
    type: nullableText(product.type),
    sku: nullableText(product.sku),
    name: nullableText(product.name),
    status: nullableText(product.status),
    catalog_visibility: nullableText(product.catalog_visibility),
    price: nullableText(product.price),
    regular_price: nullableText(product.regular_price),
    sale_price: nullableText(product.sale_price),
    stock_status: nullableText(product.stock_status),
    stock_quantity: nullableInteger(product.stock_quantity),
    manage_stock: product.manage_stock === true,
    category_ids: readIds(product.categories),
    attributes: sanitizeAttributes(product.attributes),
    date_created_gmt: nullableText(product.date_created_gmt),
    date_modified_gmt: nullableText(product.date_modified_gmt),
  };
}

function sanitizeVariation(variation) {
  return {
    id: variation.id,
    parent_id: nullablePositiveInteger(variation.parent_id),
    sku: nullableText(variation.sku),
    status: nullableText(variation.status),
    price: nullableText(variation.price),
    regular_price: nullableText(variation.regular_price),
    sale_price: nullableText(variation.sale_price),
    stock_status: nullableText(variation.stock_status),
    stock_quantity: nullableInteger(variation.stock_quantity),
    manage_stock: variation.manage_stock === true,
    attributes: sanitizeAttributes(variation.attributes),
    date_created_gmt: nullableText(variation.date_created_gmt),
    date_modified_gmt: nullableText(variation.date_modified_gmt),
  };
}

function sanitizeRefund(refund) {
  return {
    id: refund.id,
    date_created_gmt: nullableText(refund.date_created_gmt),
    amount_micros: Math.abs(money(refund.total)),
    reason_present: Boolean(nullableText(refund.reason)),
    refunded_by: nullablePositiveInteger(refund.refunded_by),
    line_items: sanitizeRefundLines(refund.line_items),
  };
}

function sanitizeTaxes(values) {
  return (Array.isArray(values) ? values : []).map((tax) => ({
    id: nullablePositiveInteger(tax?.id),
    total: nullableText(tax?.total),
    subtotal: nullableText(tax?.subtotal),
  }));
}

function sanitizeAttributes(values) {
  return (Array.isArray(values) ? values : []).map((attribute) => ({
    id: nullablePositiveInteger(attribute?.id),
    name: nullableText(attribute?.name),
    option: nullableText(attribute?.option),
    position: nullableInteger(attribute?.position),
    visible: attribute?.visible === true,
    variation: attribute?.variation === true,
  }));
}

function sanitizeRefundLines(values) {
  return (Array.isArray(values) ? values : []).map((line) => ({
    id: nullablePositiveInteger(line?.id),
    quantity: nullableInteger(line?.quantity),
    refund_total: nullableText(line?.refund_total),
    refund_tax: nullableText(line?.refund_tax),
  }));
}
...
function buildRefundByLine(refunds) {
  ...
  for (const refund of refunds) {
    for (const line of Array.isArray(refund?.line_items) ? refund.line_items : []) {
      ...
      if (!id) continue;
      const current = result.get(id) ?? { quantity: 0, micros: 0 };
...
function readRefunds(orderId, source, fallback) {
  if (source instanceof Map && source.has(orderId)) return source.get(orderId) ?? [];
  if (isPlainObject(source) && Array.isArray(source[orderId])) return source[orderId];
  return Array.isArray(fallback) ? fallback : [];
}

function normalizeShippingMethods(values) {
  const lines = Array.isArray(values) ? values : [];
  return {
    ids: lines.map((line) => nullableText(line?.method_id)).filter(Boolean),
    titles: lines.map((line) => nullableText(line?.method_title)).filter(Boolean),
  };
}
...
function sumOrderMetrics(orders) {
  ...
  totals.cancelled_orders = 0;
  totals.failed_orders = 0;
  totals.refunded_orders = 0;
  for (const order of orders) {
    for (const field of fields) totals[field] += integerOrZero(order[field]);
    if (order.status_class === 'cancelled') totals.cancelled_orders += 1;
    if (order.status_class === 'failed') totals.failed_orders += 1;
    if (order.status === 'refunded' || integerOrZero(order.refund_micros) > 0) totals.refunded_orders += 1;
  }
  return totals;
}

function customerAggregateKey(accountKey, customerId, currency) {
  return `woocommerce:${accountKey}:registered:${customerId}:${currency}`;
}
...
function readIds(values) {
  return (Array.isArray(values) ? values : [])
    .map((value) => nullablePositiveInteger(value?.id ?? value))
    .filter(Boolean);
}
...
function requireCurrency(value, fieldName) {
  const currency = requireText(value, fieldName).toUpperCase();
  if (!/^[A-Z]{3}$/u.test(currency)) throw contractError(`${fieldName} must be an ISO currency`, fieldName);
  return currency;
}

function optionalCurrency(value) {
  const text = nullableText(value);
  return text === null ? null : requireCurrency(text, 'currency');
}
...
function requireDataset(value) {
  ...
  if (!Object.values(WOOCOMMERCE_DATASETS).includes(dataset)) {
    throw contractError('Unsupported WooCommerce dataset', 'dataset');
  }
  return dataset;
}
...
function positiveInteger(value, fieldName) {
  ...
  if (!Number.isSafeInteger(number) || number <= 0) throw contractError(`${fieldName} must be positive`, fieldName);
  return number;
}
...
function boundedInteger(value, fieldName, minimum, maximum) {
  ...
  if (!Number.isSafeInteger(number) || number < minimum || number > maximum) {
    throw contractError(`${fieldName} is out of range`, fieldName);
  }
  return number;
}
