...
export function upgradeLarkNativeAiExecutiveWriterEvidence(input = {}) {
  ...
  if (metricSummaryJson.length > maxMetricSummaryChars) {
    throw qualityError(
      'Executive Writer V8 evidence exceeds the reviewed metric-summary budget',
      'LARK_AI_EXECUTIVE_WRITER_V8_METRIC_LIMIT_EXCEEDED',
  ...
  if (channelStatusVectorJson.length > maxStatusVectorChars) {
    throw qualityError(
      'Executive Writer V8 status vector exceeds the reviewed budget',
      'LARK_AI_EXECUTIVE_WRITER_V8_STATUS_LIMIT_EXCEEDED',
