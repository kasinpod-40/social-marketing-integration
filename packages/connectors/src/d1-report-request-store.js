...
/** Shared durable report-request lifecycle for every platform adapter. */
export class D1ReportRequestStore {
  constructor(input = {}) {
    this.db = requireD1(input.db);
    this.now = typeof input.now === 'function' ? input.now : () => Date.now();
    this.defaultPlatformScope = input.defaultPlatformScope ?? null;
  }

  async claim(input = {}) {
    const row = requireRequest({
      ...input,
      platformScope: input.platformScope ?? this.defaultPlatformScope,
    }, this.now());
    let result;
    try {
...
function requireRequest(value, now) {
  ...
  return Object.freeze({
    ...
    platformScope: requireText(value.platformScope, 'platformScope'),
    periodStart,
    periodEnd,
...
function assertIdentity(existing, incoming) {
  const fields = ['customerKey', 'accountKey', 'platformScope', 'periodStart', 'periodEnd', 'comparisonMode'];
...
/** Backward-compatible TikTok adapter over the shared report request lifecycle. */
export class D1TikTokReportRequestStore extends D1ReportRequestStore {
  constructor(input = {}) {
    super({ ...input, defaultPlatformScope: 'tiktok' });
  }
}
