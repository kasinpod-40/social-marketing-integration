...
export function readLarkTableIdsFromEnv(env, requiredKeys = LARK_TABLE_KEYS) {
  ...
  for (const tableKey of requiredKeys) {
    if (logicalKeys.has(tableKey)) throw permanentError(`Duplicate Lark logical table key: ${tableKey}`, { code: 'LARK_TABLE_CONFIG_INVALID', details: { tableKey } });
    logicalKeys.add(tableKey);
    const tableId = readTableId(env, tableKey);
    const existingOwner = ownerByTableId.get(tableId);
    if (existingOwner) throw permanentError(`Lark table ID ${tableId} is assigned to both ${existingOwner} and ${tableKey}`, { code: 'LARK_TABLE_CONFIG_INVALID', details: { tableId, tableKeys: [existingOwner, tableKey] } });
...
export function readTableId(env, tableKey) {
  ...
  if (!envName) throw permanentError(`Unknown Lark table key: ${tableKey}`, { code: 'LARK_TABLE_CONFIG_INVALID', details: { tableKey } });
  const value = env?.[envName];
  if (typeof value !== 'string' || value.trim() === '') throw permanentError(`Missing required env ${envName} for Lark table ${tableKey}`, { code: 'LARK_TABLE_CONFIG_INVALID', details: { envName, tableKey } });
  return value.trim();
}
