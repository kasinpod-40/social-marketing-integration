...
async function processMetaManualJob(input, connectorKey, metaConfig) {
  ...
  const source = resolveSourceRuntime(sourceRuntime, connectorKey);
  ...
  const deterministicSyncRunId = `meta:${operation.operationId}`;
...
function resolveSourceRuntime(runtime, connectorKey) {
  ...
  const mapping = connectorKey === 'facebook'
    ...
    : connectorKey === 'instagram'
      ...
      : runtime?.mappings?.metaAdAccountId;
  if (!adapter || !mapping) {
  ...
  return Object.freeze({
    adapter,
    sourceAccountId: connectorKey === 'meta_ads'
      ? normalizeMetaAdAccountId(mapping)
      : requireJobText(mapping, `${connectorKey}.sourceAccountId`),
  });
}
